...
import { StreamClientId } from '../core/streaming/IStreamClient';
...
import { ToolOrchestrator } from '../core/tools/ToolOrchestrator'; // Corrected path
import { ToolExecutionResult, ToolExecutionContext } from '../core/tools/ITool'; // Corrected path
import { v4 as uuidv4 } from 'uuid';
import { GMIError, GMIErrorCode } from '../../utils/errors'; // Assuming this path is correct for errors
import { IPersonaDefinition } from '../cognitive_substrate/personas/IPersonaDefinition';
import { StreamingManager } from '../core/streaming/StreamingManager'; // Added for future integration
...
export interface AgentOSOrchestratorDependencies {
  ...
  streamingManager: StreamingManager; // Added streamingManager
}
...
export class AgentOSOrchestrator {
   ...
   * Key: streamClientId (generated by orchestrator for this interaction flow).
  ...
  private async pushChunkToStream(
    ...
    await this.dependencies.streamingManager.pushChunk(streamClientId, chunk);
  }
   ...
   * @returns {Promise<StreamClientId>} The ID of the stream to which responses will be pushed.
   * @throws {GMIError} If critical initialization or setup fails.
   */
  public async orchestrateTurn(input: AgentOSInput): Promise<StreamClientId> {
    this.ensureInitialized();
    const agentOSStreamClientId = await this.dependencies.streamingManager.createStream();
    console.log(`AgentOSOrchestrator: Starting turn for AgentOS Stream ${agentOSStreamClientId}, User ${input.userId}, Session ${input.sessionId}`);

    // Execute the turn processing asynchronously without awaiting it here,
    // so this method can return the streamClientId quickly.
    this._processTurnInternal(agentOSStreamClientId, input).catch(async (criticalError: any) => {
      console.error(`AgentOSOrchestrator: Critical unhandled error in _processTurnInternal for stream ${agentOSStreamClientId}:`, criticalError);
      try {
        await this.pushErrorChunk(
          agentOSStreamClientId,
        ...
        await this.dependencies.streamingManager.closeStream(agentOSStreamClientId, "Critical orchestrator error");
      } catch (cleanupError: any) {
        console.error(`AgentOSOrchestrator: Error during critical error cleanup for stream ${agentOSStreamClientId}:`, cleanupError);
      }
      this.activeStreamContexts.delete(agentOSStreamClientId);
    });

    return agentOSStreamClientId;
  }
   ...
   * @private
   */
  private async _processTurnInternal(agentOSStreamClientId: StreamClientId, input: AgentOSInput): Promise<void> {
    let gmi: IGMI | undefined;
    let conversationContext: ConversationContext | undefined;
    let currentPersonaId = input.selectedPersonaId || 'default'; // Fallback needed if config has default
    ...
    try {
      ...
      this.activeStreamContexts.set(agentOSStreamClientId, streamContext);

      await this.pushChunkToStream(
        agentOSStreamClientId, AgentOSResponseChunkType.SYSTEM_PROGRESS,
      ...
      const gmiInput = this.constructGMITurnInput(agentOSStreamClientId, input, streamContext);
      ...
      while (continueProcessing && currentToolCallIteration < this.config.maxToolCallIterations) {
        ...
        if (lastGMIOutput?.toolCalls && lastGMIOutput.toolCalls.length > 0) {
          ...
          // So, we'd take the tool_calls from that GMIOutput and then break this loop to let orchestrateToolResult handle them.
           await this.processGMIOutput(agentOSStreamClientId, streamContext, lastGMIOutput, true /*isContinuation*/);
            if (lastGMIOutput.toolCalls && lastGMIOutput.toolCalls.length > 0) {
            ...
            console.warn(`AgentOSOrchestrator: GMI output after tool result was not final and had no tool calls. Ending turn for stream ${agentOSStreamClientId}.`);
        ...
        for await (const gmiChunk of gmiStreamIterator) {
          await this.transformAndPushGMIChunk(agentOSStreamClientId, streamContext, gmiChunk);
      ...
      if (currentToolCallIteration >= this.config.maxToolCallIterations && continueProcessing) {
        console.warn(`AgentOSOrchestrator: Max tool call iterations reached for stream ${agentOSStreamClientId}. Forcing termination.`);
        await this.pushErrorChunk(
          agentOSStreamClientId, currentPersonaId, gmiInstanceIdForChunks,
      ...
      const finalGMIStateForResponse = lastGMIOutput || (gmi ? await gmi.getOverallHealth().then(h => ({ responseText: "Processing complete.", isFinal: true, usage: h.memoryHealth?.ragSystemStats?.details?.usage /* placeholder */ })) : {responseText: "Processing ended.", isFinal: true});

      await this.pushChunkToStream(
        agentOSStreamClientId, AgentOSResponseChunkType.FINAL_RESPONSE,
        gmiInstanceIdForChunks, currentPersonaId, true,
        {
          finalResponseText: finalGMIStateForResponse?.responseText,
          ...
          usage: finalGMIStateForResponse?.usage,
          ...
          activePersonaDetails: gmi ? gmi.getCurrentPersonaDefinition() : undefined,
        }
      );

    } catch (error: any) {
      const gmiErr = GMIError.wrap?.(error, GMIErrorCode.GMI_PROCESSING_ERROR, `Error in orchestrateTurn for stream ${agentOSStreamClientId}`) ||
                     new GMIError(`Error in orchestrateTurn for stream ${agentOSStreamClientId}: ${error.message}`, GMIErrorCode.GMI_PROCESSING_ERROR, error);
      console.error(`AgentOSOrchestrator: Error during _processTurnInternal for stream ${agentOSStreamClientId}:`, gmiErr);
      await this.pushErrorChunk(
          agentOSStreamClientId, currentPersonaId, gmiInstanceIdForChunks,
      ...
      this.activeStreamContexts.delete(agentOSStreamClientId);
      console.log(`AgentOSOrchestrator: Finished processing for AgentOS Stream ${agentOSStreamClientId}. Context removed.`);
    }
  }
   ...
   * @public
   * @async
   * @param {string} agentOSStreamClientId - The orchestrator's stream ID for this interaction flow.
  ...
  public async orchestrateToolResult(
    ...
    const streamContext = this.activeStreamContexts.get(agentOSStreamClientId);
    if (!streamContext) {
      const errMsg = `Orchestrator: Received tool result for unknown or inactive streamClientId: ${agentOSStreamClientId}. Tool: ${toolName}, CallID: ${toolCallId}`;
      console.error(errMsg);
      // Cannot push to a non-existent stream context. This is a critical failure.
      throw new GMIError(errMsg, GMIErrorCode.RESOURCE_NOT_FOUND, { agentOSStreamClientId, toolCallId });
    }
    ...
    console.log(`AgentOSOrchestrator: Feeding tool result for stream ${agentOSStreamClientId}, GMI ${gmiInstanceIdForChunks}, tool call ${toolCallId} (${toolName}) back to GMI.`);

    try {
      // Emit the tool result itself as a chunk
      await this.pushChunkToStream(
        agentOSStreamClientId, AgentOSResponseChunkType.TOOL_RESULT_EMISSION,
      ...
      await this.processGMIOutput(agentOSStreamClientId, streamContext, gmiOutputAfterTool, false);
      ...
      if (gmiOutputAfterTool.toolCalls && gmiOutputAfterTool.toolCalls.length > 0) {
        await this.pushChunkToStream(
          agentOSStreamClientId, AgentOSResponseChunkType.TOOL_CALL_REQUEST,
        ...
        // Push a final response marker or the already pushed final data from processGMIOutput takes precedence.
         await this.pushChunkToStream(
            ...
            gmiInstanceIdForChunks, personaId, true,
            {
              ...
              usage: gmiOutputAfterTool.usage,
        ...
        this.activeStreamContexts.delete(agentOSStreamClientId); // Clean up context for this completed flow
        await this.dependencies.streamingManager.closeStream(agentOSStreamClientId, "Tool processing complete and final response generated.");
      }
      ...
      const gmiErr = GMIError.wrap?.(error, GMIErrorCode.TOOL_ERROR, `Error in orchestrateToolResult for stream ${agentOSStreamClientId}`) ||
                     new GMIError(`Error in orchestrateToolResult for stream ${agentOSStreamClientId}: ${error.message}`, GMIErrorCode.TOOL_ERROR, error);
      console.error(`AgentOSOrchestrator: Critical error processing tool result for stream ${agentOSStreamClientId}:`, gmiErr);
      await this.pushErrorChunk(
        agentOSStreamClientId, personaId, gmiInstanceIdForChunks,
        gmiErr.code, gmiErr.message, gmiErr.details
      );
      this.activeStreamContexts.delete(agentOSStreamClientId);
      await this.dependencies.streamingManager.closeStream(agentOSStreamClientId, "Critical error during tool result processing.");
  ...
  private async processGMIOutput(
      ...
      if (gmiOutput.responseText) {
          await this.pushChunkToStream(
              agentOSStreamClientId, AgentOSResponseChunkType.TEXT_DELTA,
      ...
      if (gmiOutput.uiCommands && gmiOutput.uiCommands.length > 0) {
          await this.pushChunkToStream(
              agentOSStreamClientId, AgentOSResponseChunkType.UI_COMMAND,
              gmiInstanceIdForChunks, personaId, false,
      ...
      if (gmiOutput.error) {
          ...
          if (gmiOutput.isFinal) {
              this.activeStreamContexts.delete(agentOSStreamClientId);
              await this.dependencies.streamingManager.closeStream(agentOSStreamClientId, `GMI reported an error: ${gmiOutput.error.message}`);
          }
      ...
      if (gmiOutput.isFinal && (!gmiOutput.toolCalls || gmiOutput.toolCalls.length === 0)) {
          ...
          this.activeStreamContexts.delete(agentOSStreamClientId);
          await this.dependencies.streamingManager.closeStream(agentOSStreamClientId, "Processing complete.");
      }
  }
  ...
  private async transformAndPushGMIChunk(
    ...
    switch (gmiChunk.type) {
      ...
      case GMIOutputChunkType.ERROR:
        ...
        if (gmiChunk.isFinal) {
          this.activeStreamContexts.delete(agentOSStreamClientId);
          await this.dependencies.streamingManager.closeStream(agentOSStreamClientId, `GMI stream error: ${errDetails.message || String(gmiChunk.content)}`);
        }
        break;
      case GMIOutputChunkType.FINAL_RESPONSE_MARKER:
        ...
        if (gmiChunk.isFinal) { // This marker SHOULD imply isFinal=true
            ...
            this.activeStreamContexts.delete(agentOSStreamClientId);
            await this.dependencies.streamingManager.closeStream(agentOSStreamClientId, "GMI processing complete (final marker).");
        }
        break;
      case GMIOutputChunkType.USAGE_UPDATE:
        // TODO: Could send a specific AgentOSMetadataUpdateChunk if defined, or log.
        console.log(`AgentOSOrchestrator: UsageUpdate from GMI on stream ${agentOSStreamClientId}:`, gmiChunk.content);
        break;
      default:
        console.warn(`AgentOSOrchestrator: Unhandled GMIOutputChunkType '${gmiChunk.type}' on stream ${agentOSStreamClientId}. Content:`, gmiChunk.content);
    }
  }
   ...
   * @private
   */
  private constructGMITurnInput(agentOSStreamClientId: string, input: AgentOSInput, streamContext: ActiveStreamContext): GMITurnInput {
    ...
    if (input.visionInputs && input.visionInputs.length > 0 || input.audioInput) {
        ...
        console.warn(`AgentOSOrchestrator: No primary input in AgentOSInput for stream ${agentOSStreamClientId}. Sending as system message to GMI.`);
    }

    return {
        interactionId: agentOSStreamClientId + `_turn_${uuidv4()}`, // More specific interaction ID for GMI
        userId,
  ...
  public async shutdown(): Promise<void> {
    ...
    for (const streamClientId of this.activeStreamContexts.keys()) {
        try {
            await this.dependencies.streamingManager.closeStream(streamClientId, "Orchestrator shutting down.");
        } catch (e:any) {
            console.error(`AgentOSOrchestrator: Error closing stream ${streamClientId} during shutdown: ${e.message}`);
        }
    }
