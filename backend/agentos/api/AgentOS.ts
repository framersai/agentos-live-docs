 ...
 * @see {@link ../../utils/errors.ts} for custom error definitions.
 */
...
export class AgentOSServiceError extends GMIError {
  ...
  constructor(message: string, code: GMIErrorCode | string, details?: any, componentOrigin?: string) {
    super(
      message,
      code as GMIErrorCode,
      details,
      undefined, // context - not directly set here, GMIError might have its own.
      undefined, // httpStatusCode - GMIError infers this.
      true,      // isOperational - faults in AgentOS facade are generally operational.
      componentOrigin // component - passed to GMIError constructor.
    );
...
export class AgentOS implements IAgentOS {
  ...
  public async initialize(config: AgentOSConfig): Promise<void> {
    if (this.initialized) {
      console.warn('AgentOS (Service Facade) is already initialized. Skipping re-initialization.');
      return;
    }
    ...
    try {
      ...
      // The utilityAIService from config is expected to conform to IPromptEngineUtilityAI
      // if the PromptEngine requires those specific methods.
      if (this.utilityAIService) {
        // Perform a basic structural check for required methods if utilityAIService is provided
        const peUtility = this.utilityAIService as IPromptEngineUtilityAI; // Cast for check
        if (
            ...
            // Depending on strictness, could throw new ConfigurationError here.
        }
      }
  ...
  public async *processRequest(input: AgentOSInput): AsyncGenerator<AgentOSResponse, void, undefined> {
    ...
    try {
      console.log(`AgentOS.processRequest: User '${input.userId}', Session '${input.sessionId}', Persona '${effectivePersonaId}'. Orchestrator will manage stream creation.`);
      ...
      streamIdToListen = await this.agentOSOrchestrator.orchestrateTurn(input);
      await this.streamingManager.registerClient(streamIdToListen, bridge);
      console.log(`AgentOS.processRequest: Bridge client ${bridge.id} registered to stream ${streamIdToListen}.`);
      ...
      const serviceError = AgentOSServiceError.wrap(
        ...
        `Error during AgentOS.processRequest for user '${input.userId}'`,
      ...
      if (streamIdToListen) {
        console.log(`AgentOS.processRequest: Deregistering bridge client ${bridge.id} from stream ${streamIdToListen}.`);
        await this.streamingManager.deregisterClient(streamIdToListen, bridge.id)
          .catch(deregError => console.error(`AgentOS.processRequest: Error deregistering bridge client ${bridge.id}: ${(deregError as Error).message}`));
      }
