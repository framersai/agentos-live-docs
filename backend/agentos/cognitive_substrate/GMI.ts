...
// FIXED: Corrected import path for ToolExecutor
...
export class GMI implements IGMI {
  ...
  private conversationHistory: ChatMessage[]; // FIXED: Changed from ModelChatMessage
  ...
  private validateInitializationInputs(persona: IPersonaDefinition, config: GMIBaseConfig): void {
    ...
    if (errors.length > 0) {
      // FIXED: Use GMI_INITIALIZATION_ERROR
  ...
  private ensureReady(): void {
    ...
    if (this.state !== GMIPrimeState.READY) {
      // FIXED: Assuming GMIErrorCode.INVALID_STATE is added
      throw new GMIError(
  ...
  private buildPromptExecutionContext(): PromptExecutionContext {
    ...
    // FIXED: Remove gmiInstanceId if not in PromptExecutionContext interface
    // FIXED: Add workingMemory as it's required by IPromptEngine.PromptExecutionContext
   ...
   */
  private shouldTriggerRAGRetrieval(query: string): boolean { // FIXED: Error 16 - Define this method
  ...
  /** @inheritdoc */
  // FIXED: Return type changed to AsyncGenerator<GMIOutputChunk, GMIOutput, undefined> (Error 250 in AgentOSOrchestrator.ts)
  public async *processTurnStream(turnInput: GMITurnInput): AsyncGenerator<GMIOutputChunk, GMIOutput, undefined> {
    ...
    try {
      ...
      main_processing_loop: while (safetyBreak < 5) {
        ...
        // FIXED: Error 17 - Use systemPrompts array
        ...
        // FIXED: Error 21, 27 - Model ID from persona's defaultModelId or config
        ...
        // FIXED: Error 18, 19 - Use result.prompt, selectedModel is from modelTargetInfo
        ...
        // FIXED: Use ModelCompletionOptions
        ...
        // FIXED: Use ModelCompletionResponse for chunk type
      ...
      // FIXED: Error 24, 25 (GMIError.wrap, PROCESSING_ERROR)
  ...
  public async handleToolResult(
    ...
    const systemTurnInput: GMITurnInput = {
        ...
        sessionId: this.reasoningTrace?.sessionId, // FIXED: ReasoningTrace doesn't have sessionId by default
        ...
        metadata: {
            ...
            options: turnInput.metadata?.options || {} // Carry over options if any relevant
        }
    };
    ...
    const finalGmiOutputFromStream = await (async () => {
        for await (const chunk of stream) {
            ...
            if (chunk.usage) {
                ...
                aggregatedUsage.completionTokens += chunk.usage.completionTokens || 0;
                aggregatedUsage.totalTokens = aggregatedUsage.promptTokens + aggregatedUsage.completionTokens;
                if (chunk.usage.costUSD) aggregatedUsage.totalCostUSD = (aggregatedUsage.totalCostUSD || 0) + chunk.usage.costUSD; // FIXED: totalCostUsd to costUSD
            }
  ...
  private async performPostTurnIngestion(userInput: string, gmiResponse: string): Promise<void> {
    const ragConfig = this.activePersona.memoryConfig?.ragConfig;
    // FIXED: Ensure IPersonaDefinition.ts has ingestionTriggers
    ...
    try {
      ...
      if (this.utilityAI && ingestionProcessingConfig?.summarization?.enabled) {
        const summarizationOptions: SummarizationOptions = {
          ...
          // FIXED: Use correct IPersonaDefinition fields for default model/provider
      ...
      // FIXED: Error 28 - ensure metadata values are not undefined if not allowed by MetadataValue
      ...
      // FIXED: Error 29, 30 (GMIError.wrap, RAG_INGESTION_FAILED)
  ...
  public async _triggerAndProcessSelfReflection(): Promise<void> {
    ...
    try {
      ...
      if (!responseContent || typeof responseContent !== 'string') {
        // FIXED: Use LLM_PROVIDER_ERROR (Error 31)
      ...
      if (!parsedUpdates) {
        // FIXED: Use PARSING_ERROR (Error 32)
      ...
      // FIXED: Error 33, 34 (GMIError.wrap, PROCESSING_ERROR)
      ...
      // FIXED: Error 35 - Simplified state restoration. If previous state was processing, it makes sense to return to ready.
      // Or, if specific state is needed, previousState should be one of the valid non-reflecting states.
      this.state = (previousState !== GMIPrimeState.REFLECTING && previousState !== GMIPrimeState.IDLE && previousState !== GMIPrimeState.INITIALIZING) ? previousState : GMIPrimeState.READY;
  ...
  private getModelAndProviderForLLMCall(
    ...
    if (!providerId) {
      ...
      if (foundProvider) {
        ...
        // FIXED: Error 42 - Ensure providerId is a string or throw
  ...
  public async onMemoryLifecycleEvent(event: MemoryLifecycleEvent): Promise<LifecycleActionResponse> {
    ...
    if (personaLifecycleConf?.negotiationEnabled && event.negotiable) {
      ...
      // FIXED: Error 43 - Correctly check event.type and event.proposedAction
  ...
  public async getOverallHealth(): Promise<GMIHealthReport> {
    ...
    let overallSystemHealthy = memoryHealth?.overallStatus === 'OPERATIONAL'; // FIXED: Error - memoryHealth possibly undefined
  ...
  public async shutdown(): Promise<void> {
    ...
    try {
      ...
      // FIXED: Error 45 - IPromptEngine.shutdown removed as not in interface
