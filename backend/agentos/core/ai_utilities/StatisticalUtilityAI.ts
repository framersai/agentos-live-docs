...
export class StatisticalUtilityAI implements IUtilityAI {
  ...
  constructor(utilityId?: string) {
    ...
    this.stopWords = new Map([['en', DEFAULT_ENGLISH_STOP_WORDS]]); // Default English
  ...
  public async initialize(config: StatisticalUtilityAIConfig): Promise<void> {
    if (this.isInitialized) {
      ...
      this.stopWords.clear().set('en', DEFAULT_ENGLISH_STOP_WORDS); // Reset to default
  ...
  private getSentimentAnalyzer(language: string): natural.SentimentAnalyzer {
    ...
    if (!this.sentimentAnalyzers.has(langCode)) {
        // 'natural' SentimentAnalyzer default is English, AFINN lexicon.
        // For other languages, a specific lexicon and potentially different tokenizer/stemmer needed.
        // This is a simplification; real multilingual sentiment needs more setup.
        const stemmer = (langCode === 'en') ? natural.PorterStemmer : undefined; // Only use Porter for English for 'natural's default
        const vocabulary = (langCode === 'en' && this.config.sentimentConfig?.lexiconPath)
            ? this.loadSentimentLexiconFromFile(this.config.sentimentConfig.lexiconPath)
            : undefined; // TODO: Load language-specific lexicons

        this.sentimentAnalyzers.set(langCode, new natural.SentimentAnalyzer(langCode, stemmer, 'afinn', vocabulary));
  ...
  public async calculateSimilarity(text1: string, text2: string, options?: SimilarityOptions): Promise<number> {
    ...
    if (method === 'jaccard') {
      return natural.JaroWinklerDistance(tokens1.join(' '), tokens2.join(' ')) > 0 ? natural.JaccardIndex(tokens1, tokens2) : 0; // natural's JaccardIndex needs arrays
  ...
  public async analyzeSentiment(text: string, options?: SentimentAnalysisOptions): Promise<SentimentResult> {
    ...
    const lexicon = this.sentimentAnalyzers.get(lang)?.vocabulary; // Access internal for better details
