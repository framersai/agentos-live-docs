...
import { IProvider, ModelCompletionOptions, ModelCompletionResponse, ChatMessage } from '../llm/providers/IProvider';
...
export abstract class AgentCore implements IAgent {
  ...
  protected async gatherPromptComponents(
    ...
    const historyMaxMessages = this.agentConfig.promptEngineConfigOverrides?.defaultHistoryMaxMessages ||
                               (this.promptEngine as any).config?.defaultHistoryMaxMessages || // Unsafe access, better if PromptEngine exposes
                               20; // Hard fallback
    ...
    if (availableTools && availableTools.length > 0 && this.agentConfig.modelTargetInfo.toolSupport !== 'none') {
      components.toolSchemas = availableTools.map(tool => tool.definition);
    }
    return components;
  }
  ...
  protected async callLLM(
    ...
    try {
      promptResult = await this.promptEngine.constructPrompt(
        ...
        this.agentConfig.promptEngineConfigOverrides // Pass agent specific overrides
      );
    } catch (error: any) {
    ...
    try {
      llmResponse = await llmProvider.generateCompletion(promptResult.prompt as ChatMessage[], completionOptions); // Cast as ChatMessage[]
    } catch (error: any) {
  ...
  protected parseLLMResponse(llmResponse: ModelCompletionResponse): {
    ...
    const responseText = choice.message?.content || choice.text || null;
