...
export class AudioProcessor extends EventEmitter {
  ...
  private processAudioEvent(event: AudioProcessingEvent): void {
    ...
    if (this.internalState.vadIsSpeaking || this.speechDataBuffer.length > 0) {
      ...
      if (vadResult.isSpeech || (this.isCurrentlySpeakingOrRecentlyEnded() && this.speechDataBuffer.length < ( (this.vad.getCurrentState().consecutiveSilenceFrames * this.frameDurationMs) < this.vad.config.maxSilenceDurationMsInSpeech ? 100 : 0 ) ) ) { // Heuristic to grab trailing audio frames
