...
import { ITool } from '../../tools/interfaces/ITool'; // Assuming ITool defines parameters in a JSON-schema like way.
import { VisionInputData, AudioInputData } from '../../cognitive_substrate/IGMI';
import { Message } from '../conversation/ConversationMessage'; // Assuming this is ConversationMessage from IGMI.ts/GMI.ts context
...
export interface PromptComponents {
  ...
  conversationHistory?: Message[]; // Using Message type from ConversationContext
...
export interface IPromptEngineUtilityAI {
   ...
   * @returns {Promise<{ summary: Message[]; originalTokenCount: number; finalTokenCount: number; messagesPreserved?: number; }>}
   ...
   * @returns {Promise<{ summary: string; originalTokenCount: number; finalTokenCount: number; sourcesMentioned?: string[] }>}
