...
export class OllamaProvider implements IProvider {
  ...
  public async generateCompletion(
    ...
    try {
      const response = await this.client.post<OllamaChatResponseChunk>('/chat', payload);
      ...
      const axiosError = error as AxiosError<OllamaChatResponseChunk>;
      const status = axiosError.response?.status;
      const errorData = axiosError.response?.data;
      const message = errorData?.error || axiosError.message || 'Unknown Ollama API error';
      throw new OllamaProviderError(
        message,
        'API_REQUEST_FAILED',
        status,
  ...
  public async *generateCompletionStream(
    ...
    try {
      ...
      const message = errorData?.error || axiosError.message || 'Failed to connect to Ollama stream.';
      throw new OllamaProviderError(
        message,
    ...
    const stream = responseStream.data as NodeJS.ReadableStream;
  ...
  public async generateEmbeddings(
    ...
    for (let i = 0; i < texts.length; i++) {
      ...
      try {
        const response = await this.client.post<OllamaEmbeddingResponse>('/embeddings', payload);
        embeddingsData.push({
          object: 'embedding',
        ...
        const message = (errorData?.error as string) || axiosError.message || `Failed to generate embedding for text index ${i}`;
        throw new OllamaProviderError(
          message,
          'EMBEDDING_FAILED',
          status,
  ...
  public async listAvailableModels(filter?: { capability?: string }): Promise<ModelInfo[]> {
    this.ensureInitialized();
    try {
      const response = await this.client.get<OllamaListTagsResponse>('/tags');
  ...
  public async getModelInfo(modelId: string): Promise<ModelInfo | undefined> {
    ...
    try {
      const response = await this.client.post<{modelfile: string, parameters: string, template: string, details: any}>('/show', { name: modelId });
      const details = response.data.details; // This structure can vary.
      ...
      return {
          ...baseInfo,
          description: `${baseInfo.description}. Parameters: ${response.data.parameters?.split('\n').filter(Boolean).join(', ') || 'N/A'}`
      };
