...
    let activePersonaId: string; // Will be definitively set
...
          conversationContext.addMessage(MessageRole.ASSISTANT, finalGMIOutputFromStream.responseText, { agentPersonaId: activePersonaId });
        }
        if (finalGMIOutputFromStream?.toolCalls) {
          conversationContext.addMessage(MessageRole.ASSISTANT, null, { tool_calls: finalGMIOutputFromStream.toolCalls, agentPersonaId: activePersonaId });
        }
...
          activePersonaDetails: personaDef ? { id: personaDef.id, name: personaDef.name, description: personaDef.description, version: personaDef.version } : undefined,
        }
      );
      
        // TS2554 fix: Pass only 1 or 2 arguments
...
          try {
            if (gmiOutput.responseText) {
              conversationContext.addMessage(MessageRole.ASSISTANT, gmiOutput.responseText, { agentPersonaId: personaId });
            }
            await this.dependencies.conversationManager.saveConversation(conversationContext);
