...
function createPersonaLoaderConfig(env: EnvironmentConfig): PersonaLoaderConfig {
  return {
    personaDirectoryPath: './personas', // Default personas directory
    enableFileWatching: env.NODE_ENV === 'development',
    validationLevel: 'strict',
    defaultPersonaId: env.DEFAULT_PERSONA_ID,
 ...
 */
async function createAuthService(prisma: PrismaClient, env: EnvironmentConfig): Promise<IAuthService> {
  const authService = new AuthService(prisma);
  
  await authService.initialize({
    jwtSecret: env.JWT_SECRET,
    jwtExpiresIn: env.JWT_EXPIRES_IN,
    apiKeyEncryptionKeyHex: env.API_KEY_ENCRYPTION_KEY_HEX,
...
async function createSubscriptionService(
  ...
  const lemonSqueezyService = createLemonSqueezyService(env);
  const subscriptionService = new SubscriptionService(prisma, authService, lemonSqueezyService);
...
export async function createAgentOSConfig(): Promise<AgentOSConfig> {
  ...
  try {
    ...
    console.log('AgentOS Config: Database connection established');

    // Create and initialize services
    const authService = await createAuthService(prisma, env);
    const subscriptionService = await createSubscriptionService(prisma, authService, env);
    ...
    console.log(`AgentOS Config: Default persona: ${env.DEFAULT_PERSONA_ID}`);
