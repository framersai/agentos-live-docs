...
class LLMContextAggregatorServiceImpl implements IContextAggregatorService {
  ...
  constructor() {
    this.loadSystemPrompt()
      .then(() =>
        console.log('[LLMContextAggregatorService] Initialized and system prompt loaded.')
      )
      .catch((err) =>
        console.error(
          '[LLMContextAggregatorService] CRITICAL: Failed to load system prompt on init:',
          err
        )
      );
    // Ensure LLM services are initialized, as this service depends on callLlm
    initializeLlmServices().catch((err) =>
      console.error(
        '[LLMContextAggregatorService] Error during dependent LLM service initialization:',
        err
      )
    );
  }
  ...
  private async loadSystemPrompt(): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[LLMContextAggregatorService] Failed to load system prompt from ${promptPath}:`,
        error
      );
  ...
  async generateContextBundle(sources: IContextAggregatorInputSources): Promise<IContextBundle> {
    if (!this.systemPrompt) {
      ...
      if (!this.systemPrompt) {
        // Still not loaded
        throw new Error(
          '[LLMContextAggregatorService] System prompt for context aggregator is not available.'
        );
      }
    }
    ...
    if (aggregatorModelId.includes('/')) {
      ...
      if (
        Object.values(LlmProviderId).includes(potentialProvider) &&
        llmConfigService.isProviderAvailable(potentialProvider)
      ) {
    ...
    const providerIdToUse =
      providerIdFromModel || llmConfigService.getDefaultProviderAndModel().providerId;

    console.log(
      `[LLMContextAggregatorService] Calling aggregator LLM (${aggregatorModelId} via ${providerIdToUse}) with tool ${this.CONTEXT_BUNDLE_FUNCTION_NAME}.`
    );
        ...
        if (toolCall.function.name === this.CONTEXT_BUNDLE_FUNCTION_NAME) {
          try {
            ...
            if (!bundle.primaryTask || !bundle.discernmentOutcome) {
              throw new Error(
                'Invalid context bundle structure: missing primaryTask or discernmentOutcome.'
              );
            }
            console.log(
              `[LLMContextAggregatorService] Context Bundle successfully generated via function call. Discernment: ${bundle.discernmentOutcome}`
            );
            return bundle;
          } catch (e: any) {
            console.error(
              '[LLMContextAggregatorService] Error parsing ContextBundle from LLM function arguments:',
              e.message,
              `Raw arguments: ${toolCall.function.arguments}`
            );
            throw new Error(
              `LLMContextAggregatorService: Failed to parse context bundle from LLM: ${e.message}`
            );
          }
        } else {
          throw new Error(
            `LLMContextAggregatorService: LLM called an unexpected tool: ${toolCall.function.name}`
          );
        }
      } else if (response.text) {
        // Fallback attempt: LLM might have ignored function calling and returned JSON in text
        console.warn(
          '[LLMContextAggregatorService] LLM did not use function calling as instructed. Attempting to parse text response as JSON bundle.'
        );
        try {
          ...
          console.log(
            `[LLMContextAggregatorService] Context Bundle successfully parsed from text response. Discernment: ${bundle.discernmentOutcome}`
          );
          return bundle;
        } catch (e) {
          console.error(
            '[LLMContextAggregatorService] Failed to parse text response as ContextBundle JSON. Text response:',
            response.text.substring(0, 500) + '...'
          );
          throw new Error(
            'LLMContextAggregatorService: LLM response was not a valid context bundle function call or parseable JSON text.'
          );
        }
      } else {
        throw new Error(
          'LLMContextAggregatorService: LLM response contained no text and no tool calls for context bundle generation.'
        );
      }
    } catch (error: any) {
      console.error(
        '[LLMContextAggregatorService] Error calling LLM for context aggregation:',
        error
      );
