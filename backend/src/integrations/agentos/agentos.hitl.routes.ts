 ...
 *
 * This router is intentionally generic:
 * - Agents (running in-process) request approvals via the injected HITL manager.
 * - Humans/UI approve/reject via these endpoints.
 * - Optional outbound webhook notifications are configured in `agentos.hitl.service.ts`.
 */
...
const requireHitlSecret = (req: Request, res: Response, next: NextFunction): void => {
  const { enabled, secret } = hitlAuthRequired();
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const respondedBy = typeof req.body?.respondedBy === 'string' ? req.body.respondedBy : 'user';
