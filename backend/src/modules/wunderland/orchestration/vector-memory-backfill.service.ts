...
export class VectorMemoryBackfillService {
  ...
  startEnclaveEmbeddingsBackfill(
    config: EnclaveEmbeddingsBackfillConfig
  ): EnclaveEmbeddingsBackfillState {
  ...
  private async runEnclaveBackfill(config: EnclaveEmbeddingsBackfillConfig): Promise<void> {
    ...
    const selectedEnclaves = (() => {
      ...
      return allEnclaves.filter(
        (e) =>
          wanted.has(String(e.enclave_id).toLowerCase()) || wanted.has(String(e.name).toLowerCase())
      );
    })();

    const targets: Array<{ enclaveId: string | null; enclaveName: string }> = selectedEnclaves.map(
      (e) => ({
