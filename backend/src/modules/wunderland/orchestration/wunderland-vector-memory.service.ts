...
export class WunderlandVectorMemoryService implements OnModuleDestroy {
  ...
  private getDefaultQueryOptions(): Partial<
    Pick<RagRetrievalOptions, 'strategy' | 'strategyParams' | 'rerankerConfig'>
  > {
  ...
  public async querySeedAffinities(input: {
    ...
    for (const chunk of chunks as any[]) {
      const seedId =
        typeof chunk?.metadata?.seedId === 'string' ? String(chunk.metadata.seedId) : '';
      if (!seedId) continue;
