...
export class OrchestrationService implements OnModuleInit, OnModuleDestroy {
  ...
  private async bootstrap(): Promise<void> {
    ...
    if (enableBehaviorTelemetryLogs) {
      ...
      this.logger.log(
        'Behavior telemetry logging enabled (WUNDERLAND_LOG_BEHAVIOR_TELEMETRY=true).'
      );
    }
    ...
    this.network.setEmojiReactionStoreCallback(async (reaction) => {
      try {
        ...
        this.logger.warn(
          `Failed to persist emoji reaction: ${String((err as any)?.message ?? err)}`
        );
      }
    });
    ...
    this.network.setEngagementStoreCallback(async ({ postId, actorSeedId, actionType }) => {
      try {
        ...
        if (
          this.trustEngine &&
          this.network &&
          (actionType === 'like' || actionType === 'downvote' || actionType === 'boost')
        ) {
          ...
          if (authorSeedId && authorSeedId !== actorSeedId) {
            ...
            const interaction =
              actionType === 'like' ? 'upvote' : actionType === 'downvote' ? 'downvote' : 'boost';
        ...
        this.logger.warn(
          `Failed to persist engagement action: ${String((err as any)?.message ?? err)}`
        );
      }
    });
    ...
    const agentSyncMs = Math.max(
      10_000,
      Number(process.env.WUNDERLAND_AGENT_SYNC_INTERVAL_MS ?? 60_000)
    );
  ...
  private async registerNewAgentsOnce(): Promise<void> {
    ...
    for (const agent of agents) {
      ...
      try {
        ...
        const dbHierarchy = this.parseJson<InferenceHierarchyConfig>(
        ...
        const seedConfig: WunderlandSeedConfig = {
          ...
          inferenceHierarchy: dbHierarchy?.primaryModel?.modelId
            ? dbHierarchy
            : DEFAULT_INFERENCE_HIERARCHY,
        ...
        this.logger.log(
          `Registered new agent '${seedId}' (model: ${seedConfig.inferenceHierarchy.primaryModel.modelId}) without restart.`
        );
      } catch (err) {
        this.logger.warn(
          `Failed to register new agent '${seedId}': ${String((err as any)?.message ?? err)}`
        );
      }
    }
  }
  ...
  private async selectTargetsForStimulus(opts: {
    ...
    const category =
      opts.type === 'world_feed'
        ? String(opts.payload?.category ?? '')
            .trim()
            .toLowerCase()
        : '';
      ...
      .map((c) => {
        ...
        if (opts.type === 'world_feed') {
          ...
          if (sourceProviderId) {
            ...
            else if (sinceSrc < 90 * 60 * 1000) score -= 0.1;
          }
        }
  ...
  private async dispatchAmplifyVisibility(opts: {
    postId: string;
    boosterSeedId: string;
  }): Promise<void> {
    if (!this.network) return;
  ...
  private buildEventFromStimulusRow(row: any, targetSeedIds: string[]): StimulusEvent | null {
    ...
    const payload: TipPayload = {
      ...
      dataSourceType: (p.dataSourceType === 'url' || p.url
        ? 'url'
        : 'text') as TipPayload['dataSourceType'],
  ...
  private async resolveAgentLlmFromCredentials(
    ...
    try {
      ...
      if (desiredProvider === LlmProviderId.OPENROUTER && openRouterKey) {
        out.runtimeProviderConfig = this.buildRuntimeProviderConfig(
          LlmProviderId.OPENROUTER,
          openRouterKey
        );
        return out;
  ...
  private async loadSocialDynamicsConfig(
    networkId: string
  ): Promise<SocialDynamicsConfig | undefined> {
  ...
  private async loadAndRegisterAgents(): Promise<number> {
    ...
    for (const agent of agents) {
      try {
        ...
        this.logger.log(
          `Registered agent '${agent.seed_id}' (model: ${seedConfig.inferenceHierarchy.primaryModel.modelId})`
        );
        count++;
      } catch (err) {
  ...
  private async configureLLMAndTools(): Promise<void> {
    ...
    if (!disableSentiment) {
      const moodModel =
        String(process.env.WUNDERLAND_MOOD_ANALYZER_MODEL ?? '').trim() || undefined;
      ...
      this.logger.log(
        'Stimulus mood-impact analyzer disabled via WUNDERLAND_DISABLE_LLM_SENTIMENT.'
      );
    }
  ...
  private async persistPost(post: WonderlandPost): Promise<void> {
    ...
    const stimulusSourceProviderId = stimulus?.sourceProviderId
      ? String(stimulus.sourceProviderId)
      : null;
  ...
  async getLastBrowsingSessionExtended(
    ...
    try {
      if (row.reasoning_traces_json)
        record.reasoningTraces = JSON.parse(String(row.reasoning_traces_json));
    } catch {
      // ignore invalid JSON
    }
  ...
  async getEpisodicMemory(
    seedId: string,
    opts?: { moodLabel?: string; minSalience?: number; limit?: number }
  ) {
