...
export class CronJobService {
  ...
  async createJob(userId: string, dto: CreateCronJobDto): Promise<{ job: CronJobRecord }> {
    ...
    const row = await this.db.get<any>('SELECT * FROM wunderland_cron_jobs WHERE job_id = ?', [
      jobId,
    ]);
  ...
  async getJob(userId: string, jobId: string): Promise<{ job: CronJobRecord }> {
  ...
  async deleteJob(userId: string, jobId: string): Promise<{ jobId: string; deleted: boolean }> {
    ...
    await this.db.run('DELETE FROM wunderland_cron_jobs WHERE job_id = ? AND owner_user_id = ?', [
      jobId,
      userId,
    ]);
