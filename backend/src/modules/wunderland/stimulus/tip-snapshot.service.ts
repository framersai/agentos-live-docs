...
export class TipSnapshotService {
  ...
  private readonly fetchTimeoutMs = Math.max(
    1_000,
    Number(process.env.WUNDERLAND_TIP_FETCH_TIMEOUT_MS ?? 10_000)
  );
  ...
  async previewAndPin(opts: {
    content: string;
    sourceType: 'text' | 'url';
  }): Promise<TipSnapshotPreview> {
    if (!this.ipfsApiUrl) {
      throw new ServiceUnavailableException(
        'IPFS pinning not configured (set WUNDERLAND_IPFS_API_URL).'
      );
    }
  ...
  private async buildSnapshot(opts: {
    content: string;
    sourceType: 'text' | 'url';
  }): Promise<TipSnapshotV1> {
  ...
  private async fetchAndSanitizeUrl(
    urlString: string
    ...
    try {
      while (true) {
        ...
        if (total > this.maxSnapshotBytes) {
          throw new BadRequestException(
            `Content too large: exceeded ${this.maxSnapshotBytes} bytes.`
          );
        }
        chunks.push(value);
      }
  ...
  private async pinRawBlockToIpfs(content: Buffer, expectedCid: string): Promise<void> {
    ...
    if (actualCid !== expectedCid) {
      ...
      throw new BadRequestException(
        'IPFS CID mismatch: raw block CID does not match sha256-derived CID.'
      );
    }
    ...
    if (!pinRes.ok) {
      ...
      this.logger.warn(
        `IPFS pin/add failed for ${expectedCid}: ${pinRes.status} ${pinRes.statusText} ${text}`.trim()
      );
    }
  }
}
