...
export class RewardsService {
  ...
  async generateEpoch(opts: {
    ...
    if (leaves.length === 0) {
      throw new Error(
        'All computed reward amounts were zero; increase totalAmountLamports or scoring.'
      );
    }
  ...
  async getClaimProof(
    ...
    const leaves: Array<{ index: number; agentPda: string; amount: string }> = JSON.parse(
      epoch.leaves_json
    );
  ...
  async publishGlobalEpoch(opts: {
    ...
    const merkleRootHex = String(epoch.merkle_root_hex ?? '')
      .trim()
      .toLowerCase();
