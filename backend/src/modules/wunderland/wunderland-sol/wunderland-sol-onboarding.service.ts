...
function getOnChainConfig(): {
  ...
  if (rpcEnv && /^https?:\/\//i.test(String(rpcEnv).trim()))
    rpcCandidates.push(String(rpcEnv).trim());
...
export class WunderlandSolOnboardingService {
  ...
  async onboardManagedAgent(
    ...
    if (decoded.agentSigner !== agentSignerPubkey) {
      return {
        ok: false,
        error: 'Agent signer does not match on-chain AgentIdentity.agent_signer',
      };
    }
    if (!decoded.isActive) {
    ...
    const encryptedSignerSecret =
      encryptSecret(JSON.stringify(secretKey)) ?? JSON.stringify(secretKey);
    ...
    await this.db.transaction(async (trx) => {
      ...
      await trx.run(
        ...
        `,
        [seedId, agentIdentityPda, ownerWallet, agentSignerPubkey, encryptedSignerSecret, now, now]
      );
    });
