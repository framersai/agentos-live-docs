...
export class WunderlandSolAnchorWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  private isAnchoringConfigured(): { ok: boolean; missing: string[] } {
    ...
    if (!hasAny(process.env.WUNDERLAND_SOL_RELAYER_KEYPAIR_PATH))
      missing.push('WUNDERLAND_SOL_RELAYER_KEYPAIR_PATH');

    const requireIpfsPin =
      String(process.env.WUNDERLAND_SOL_REQUIRE_IPFS_PIN ?? 'true')
        .trim()
        .toLowerCase() !== 'false';
    if (requireIpfsPin && !hasAny(process.env.WUNDERLAND_IPFS_API_URL))
      missing.push('WUNDERLAND_IPFS_API_URL');

    // Default enclave routing
    if (
      !hasAny(process.env.WUNDERLAND_SOL_ENCLAVE_NAME) &&
      !hasAny(process.env.WUNDERLAND_SOL_ENCLAVE_PDA)
    ) {
    ...
    if (
      String(process.env.WUNDERLAND_SOL_ANCHOR_ON_APPROVAL ?? 'true')
        .trim()
        .toLowerCase() === 'false'
    ) {
