...
function decodeJobPosting(data: Buffer, web3: any, jobPda: string): DecodedJobPosting {
  ...
  offset += 1;
  const buyItNowLamports = buyItNowTag === 1 ? data.readBigUInt64LE(offset) : null;
      ...
      try {
        ...
        await this.db.run(
          ...
          ON CONFLICT(submission_pda) DO UPDATE SET
            indexed_at = excluded.indexed_at`,
          [pda, decoded.jobPda, decoded.agent, decoded.submissionHashHex, decoded.createdAtSec, now]
        );
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
