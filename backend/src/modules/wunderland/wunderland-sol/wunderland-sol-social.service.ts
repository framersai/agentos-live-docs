...
function rowToApiPost(row: SolPostRow): SolPostApi {
  ...
  return {
    ...
    createdSlot:
      row.created_slot === null || row.created_slot === undefined
        ? undefined
        : Number(row.created_slot),
  };
}
...
export class WunderlandSolSocialService {
  ...
  private async getOffChainPosts(opts: {
    ...
    if (qRaw) {
      const like = `%${qRaw}%`;
      where.push(
        "(LOWER(COALESCE(wp.content, '')) LIKE ? OR LOWER(COALESCE(wp.seed_id, '')) LIKE ?)"
      );
    ...
    const total = Number(totalRow?.count ?? 0);
    this.logger.debug?.(
      `Off-chain fallback: ${total} matching posts (kind=${opts.kind}, where clauses=${where.length})`
    );
    ...
    if (sort === 'top') {
      orderSql =
        'ORDER BY (COALESCE(wp.likes, 0) - COALESCE(wp.downvotes, 0)) DESC, wp.created_at DESC';
    }
  ...
  private async mergeOffChainIfStale(
    ...
    try {
      ...
      this.logger.warn(
        `Off-chain merge failed: ${err instanceof Error ? err.message : String(err)}`
      );
      return { posts: indexPosts, total: indexTotal, source: 'index' };
    }
  }
  ...
  async getPostByPda(
    postPda: string,
    opts?: { includeIpfsContent?: boolean }
  ): Promise<SolPostApi | null> {
  ...
  async getThread(opts: {
    ...
    while (queue.length > 0 && included.size < maxComments) {
      ...
      const offChainAgentMap = new Map<
        string,
        { name: string; level: string; traits: SolAgentTraits }
      >();
  ...
  private async fillMissingIpfsContent(posts: SolPostApi[]): Promise<SolPostApi[]> {
    ...
    const missing = posts.filter((p) => !p.content).slice(0, this.ipfsMaxFetchPerRequest);
    ...
    try {
      ...
      for (const r of rows) {
        fetchedAtByPostId.set(
          String(r.post_pda),
          typeof r.content_fetched_at === 'number' ? r.content_fetched_at : null
        );
      }
    } catch {
  ...
  async getDbEnclaves(): Promise<{
    ...
    return {
      enclaves: rows.map((r) => {
        ...
        const createdAtMs =
          typeof r.created_at === 'number' ? r.created_at : Date.parse(String(r.created_at));
        return {
          name: r.name,
