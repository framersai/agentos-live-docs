...
export class WunderlandSolService {
  ...
  async getAgentIdentityPda(seedId: string): Promise<string | null> {
    ...
    const pdaFromDb =
      row?.agent_identity_pda && typeof row.agent_identity_pda === 'string'
        ? row.agent_identity_pda.trim()
        : '';
  ...
  async publishGlobalRewardsEpoch(opts: {
    ...
    const claimWindowSeconds =
      opts.claimWindowSeconds != null ? BigInt(opts.claimWindowSeconds) : 0n;
    ...
    if (claimWindowSeconds < 0n)
      return { success: false, error: 'claimWindowSeconds must be >= 0' };
  ...
  scheduleCastVote(opts: { postId: string; actorSeedId: string; value: 1 | -1 }): void {
    ...
    void this.castVoteByPostId(opts.postId, opts.actorSeedId, opts.value)
      .catch((err) => {
        const message = err instanceof Error ? err.message : String(err);
        this.logger.warn(
          `Sol cast_vote failed (post=${opts.postId}, actor=${opts.actorSeedId}): ${message}`
        );
      })
      .finally(() => {
        this.inFlight.delete(key);
  ...
  private async castVoteByPostId(
    postIdOrSolPostPda: string,
    actorSeedId: string,
    value: 1 | -1
  ): Promise<void> {
  ...
  private async resolveManagedAgentSigner(
    seedId: string,
    web3: any
  ): Promise<{ ok: true; agentIdentityPda: any; agentSigner: any } | { ok: false; error: string }> {
    const raw = safeString(seedId).trim();
    ...
    if (dbRow?.encrypted_signer_secret_key && dbRow.agent_identity_pda) {
      const decrypted =
        decryptSecret(String(dbRow.encrypted_signer_secret_key)) ??
        String(dbRow.encrypted_signer_secret_key);
      const secretKeyJson = this.parseAgentSignerSecretKeyJson(decrypted);
      if (!secretKeyJson) {
        return {
          ok: false,
          error: 'Invalid stored agent signer secret (could not parse 64-byte JSON array)',
        };
      }
      ...
      try {
        ...
        return {
          ok: false,
          error: `Invalid stored agent signer secret: ${err instanceof Error ? err.message : String(err)}`,
        };
      }
    }
    ...
    try {
      ...
      return {
        ok: false,
        error: `Failed to load legacy agent signer keypair: ${err instanceof Error ? err.message : String(err)}`,
      };
    }
  }
  ...
  private async anchorPostById(postId: string): Promise<void> {
    ...
    if (shouldAnchorAsComment) {
      ...
      if (!parentPostPdaRaw) {
        await this.fail(
          postId,
          `Parent entry is not anchored on-chain yet (reply_to_post_id=${replyToPostId}).`
        );
        return;
      }
      ...
      if (!enclavePda) {
        await this.fail(
          postId,
          `Failed to resolve enclave for reply target (reply_to_post_id=${replyToPostId}).`
        );
        return;
      }
    } else {
  ...
  private async anchorCommentById(commentId: string): Promise<void> {
    ...
    if (!this.relayerKeypairPath) {
      await this.db.run(
        ...
        [
          'missing_config' satisfies AnchorStatus,
          'Missing WUNDERLAND_SOL_RELAYER_KEYPAIR_PATH.',
          commentId,
        ]
      );
      return;
    ...
    if (!parentPostPda) {
      await this.db.run(
        ...
        [
          'missing_config',
          'Parent post is not anchored on-chain yet (missing sol_post_pda).',
          commentId,
        ]
      );
      return;
    ...
    const enclavePda =
      parentEnclavePda || (await this.resolveDefaultEnclavePda({ sdk, web3 })) || '';
    if (!enclavePda) {
      await this.db.run(
        `UPDATE wunderland_comments SET anchor_status = ?, anchor_error = ? WHERE comment_id = ?`,
    ...
    await this.db.run(`UPDATE wunderland_comments SET anchor_status = ? WHERE comment_id = ?`, [
      'anchoring',
      commentId,
    ]);
  ...
  private async pinRawBlockToIpfsImpl(content: Buffer, expectedCid: string): Promise<void> {
    ...
    if (!putRes.ok) {
      ...
      throw new Error(
        `IPFS block/put failed: ${putRes.status} ${putRes.statusText} ${text}`.trim()
      );
    }
