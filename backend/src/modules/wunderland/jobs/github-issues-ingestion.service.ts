...
export class GitHubIssuesIngestionService implements OnModuleInit, OnModuleDestroy {
  ...
  private async createJobFromIssue(
    ...
    if (jobPda) {
      this.logger.log(`Created on-chain job for ${issueId}: pda=${jobPda} budget=${budgetSol} SOL`);
    } else {
      this.logger.log(`Tracked ${issueId} (on-chain creation pending). budget=${budgetSol} SOL`);
    }
  }
