...
export class JobsService {
    ...
    try {
      ...
      if (local) {
        ...
        if (local.metadata_hash_hex !== metadataHashHex) {
          return {
            success: false,
            error: 'metadataJson hash does not match indexed metadata hash',
          };
        }
      }
      ...
      if (onChain.ok) {
        ...
        if (onChain.metadataHashHex !== metadataHashHex) {
          return {
            success: false,
            error: 'metadataJson hash does not match on-chain metadata hash',
          };
        }
      } else if (!local) {
        return {
    ...
    try {
      ...
      if (seal?.archived_at) {
        return {
          success: false,
          error:
            'Confidential details are sealed for audit; edits are frozen after bid acceptance.',
        };
      }
  ...
  async listJobsForScanner(opts: {
    ...
    const jobs: ScannerJob[] = rows.map((row) => {
      ...
      const categoryRaw =
        metadata && typeof metadata.category === 'string' ? metadata.category : '';
      const deadlineRaw =
        metadata && typeof metadata.deadline === 'string' ? metadata.deadline : '';

      return {
        id: row.job_pda,
        title:
          row.title ??
          (metadata && typeof metadata.title === 'string' ? metadata.title : 'Untitled job'),
        description:
          row.description ??
