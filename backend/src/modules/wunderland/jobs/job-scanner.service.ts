...
function normalizeHexaco(traits: any): AgentProfile['hexaco'] {
  return {
    ...
    conscientiousness:
      typeof traits?.conscientiousness === 'number' ? traits.conscientiousness : 0.5,
...
export class JobScannerService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly maxAgents = Math.max(
    1,
    Math.min(250, Number(process.env.JOB_SCANNING_MAX_AGENTS ?? 50))
  );
  ...
  private async startScannersForActiveAgents(): Promise<void> {
    ...
    for (const agent of agents) {
      ...
      try {
        ...
        this.logger.warn(
          `Failed to start scanner for "${seedId}": ${String((err as any)?.message ?? err)}`
        );
      }
    }
  }
  ...
  private async startScannerForAgent(agent: {
    ...
    const state = await this.loadOrCreateAgentJobState(
      agent.seedId,
      agent.level,
      agent.reputation100
    );
  ...
  private async handleBidDecision(
    ...
    const bidLamports = (() => {
      if (
        evaluation.useBuyItNow &&
        typeof job.buyItNowLamports === 'number' &&
        job.buyItNowLamports > 0
      ) {
  ...
  private async withdrawBid(
    seedId: string,
    params: { agentId: string; bidId: string; jobId: string }
  ) {
