...
export class JobExecutionService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    ...
    if (!this.agentId) {
      this.logger.warn('Job execution enabled but JOB_EXECUTION_AGENT_ID not set. Skipping.');
      return;
    }
    ...
    const _deliverableManager = new DeliverableManager({
    ...
    this.jobExecutor = new JobExecutor({
      ...
      maxConcurrent: 1,
      maxRetries: 3,
      fetchAssignedJobs: (agentId, limit) => this.fetchAssignedJobs(agentId, limit),
      onExecutionStart: (agentId, jobId) => this.onExecutionStart(agentId, jobId),
      onExecutionComplete: (agentId, jobId, result) =>
        this.onExecutionComplete(agentId, jobId, result),
      qualityCheckerConfig: qualityChecker['threshold'] ? undefined : undefined,
   ...
   */
  private async fetchAssignedJobs(agentId: string, limit: number): Promise<AssignedJob[]> {
   ...
   */
  private async onExecutionStart(_agentId: string, jobId: string): Promise<void> {
    await this.db.run(
  ...
  private async onExecutionComplete(
    ...
    if (result.success) {
      ...
      this.logger.warn(`Execution failed for job ${jobId}: ${result.error ?? 'unknown error'}.`);
    }
  }
  ...
  private async submitJobOnChain(params: {
    ...
    if (!solStatus.enabled) {
      this.logger.debug(`Solana disabled — skipping on-chain submit for job ${params.jobId}.`);
      return {
        success: true,
  ...
  private async withdrawBid(params: {
    ...
    try {
      ...
      this.logger.log(`Bid ${params.bidId} withdrawn for job ${params.jobId} (sig: ${signature}).`);
  ...
  private async onBidInactive(params: {
    ...
    await this.db.run(`UPDATE wunderland_job_bids SET status = ? WHERE bid_pda = ?`, [
      newStatus,
      params.bidId,
    ]);
  }
