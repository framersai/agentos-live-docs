...
export class WunderlandHealthController {
  ...
  getStatus() {
    ...
    const worldFeedWebhookEnabled = Boolean(
      process.env.WUNDERLAND_WORLD_FEED_WEBHOOK_SECRET?.trim()
    );
  ...
  async getDiagnostics() {
    ...
    const requireIpfsPin =
      String(process.env.WUNDERLAND_SOL_REQUIRE_IPFS_PIN ?? 'true')
        .trim()
        .toLowerCase() !== 'false';
    if (status.solana?.enabled) {
      if (!has(process.env.WUNDERLAND_SOL_PROGRAM_ID)) envMissing.push('WUNDERLAND_SOL_PROGRAM_ID');
      if (!has(process.env.WUNDERLAND_SOL_RELAYER_KEYPAIR_PATH))
        envMissing.push('WUNDERLAND_SOL_RELAYER_KEYPAIR_PATH');
      if (requireIpfsPin && !has(process.env.WUNDERLAND_IPFS_API_URL))
        envMissing.push('WUNDERLAND_IPFS_API_URL');
      if (
        !has(process.env.WUNDERLAND_SOL_ENCLAVE_NAME) &&
        !has(process.env.WUNDERLAND_SOL_ENCLAVE_PDA)
      ) {
    ...
    const [
      ...
      count(
        `SELECT COUNT(1) as count FROM wunderbot_credentials WHERE credential_type LIKE 'LLM_API_KEY_%'`
      ),
      count(
        `SELECT COUNT(1) as count FROM wunderbot_credentials WHERE credential_type = 'LLM_MODEL'`
      ),
    ]);

    const solIndexMeta = await this.db
      .get<{
        max_indexed_at: number | null;
    ...
    const recentAnchorErrors = await this.db
      .all<{
        post_id: string;
