...
import { listExtensions, listAvailableTools, loadExtensionsRegistry, invalidateRegistryCache } from '../integrations/agentos/extensions.service.js';
...
test('listAvailableTools derives tools from registry entries', async () => {
	...
	assert.equal(tool?.extension, '@framers/agentos-research-web-search');
});
