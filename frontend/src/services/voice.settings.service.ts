...
 * @version 1.4.0 - Added sttOptions to VoiceApplicationSettings.
...
  vadThreshold: number; // Note: This was in old file's computed but not directly used in new VoiceInput.vue logic for silence. New file uses SILENCE_DBFS_THRESHOLD.
  vadSilenceTimeoutMs: number; // Note: This was in old file's computed. New file uses minSilenceDurationForSegmentEndMs directly from settings.continuousModePauseTimeoutMs for Whisper.
...
  vadThreshold: 0.15, // Retained for potential other uses or if settings UI exposes it
  vadSilenceTimeoutMs: 2500, // Retained for potential other uses
...
        changed = true;
      }
      // Ensure sttOptions and its nested properties have defaults if sttOptions itself is defined
      if (K === 'sttOptions' && this.settings.sttOptions && this.defaultSettings.sttOptions) {
...
    }
    this.applyMissingDefaults(); // Ensure nested objects like sttOptions are correctly reset or initialized
    this.loadAllTtsVoices();
