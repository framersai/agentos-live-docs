      ...
      <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-header">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h1 class="text-xl sm:text-2xl font-display font-bold flex items-center gap-2.5">
                <span class="private-title-text">Voice Chat Assistant</span>
                <span class="private-badge">
                  <ShieldCheckIconOutline class="icon-xs mr-1 text-holo-cyan" />PRO
                </span>
              </h1>
              <p class="text-xs text-primary-300/70">Full Access Environment</p>
            </div>
          </div>

          <div class="flex items-center gap-2 md:gap-3">
            <button @click="toggleGlobalMuteHandler" :title="isGloballyMuted ? 'Unmute Audio Output' : 'Mute Audio Output'"
                    class="header-icon-button" :aria-pressed="isGloballyMuted">
              <SpeakerXMarkIcon v-if="isGloballyMuted" class="icon-base" aria-hidden="true" />
              <SpeakerWaveIcon v-else class="icon-base" aria-hidden="true" />
              <span class="sr-only">{{ isGloballyMuted ? 'Unmute audio' : 'Mute audio' }}</span>
            </button>

            <div class="relative">
              <button @click="showAgentSelector = !showAgentSelector" class="btn btn-secondary btn-sm flex items-center gap-1.5 shadow-analog-outset hover:shadow-holo-sm" aria-haspopup="true" :aria-expanded="showAgentSelector">
                <component :is="activeAgent?.icon || SparklesIcon" class="icon-sm" :class="activeAgent?.iconClass || 'text-accent-light'" aria-hidden="true" />
                {{ activeAgent?.label || 'Select Agent' }}
                <ChevronDownIcon class="icon-sm transition-transform duration-200" :class="{'rotate-180': showAgentSelector}" aria-hidden="true" />
              </button>
              <transition name="dropdown-fade">
                <div v-if="showAgentSelector" class="dropdown-menu-private absolute top-full right-0 mt-2 w-72 origin-top-right z-50 glass-pane rounded-[var(--radius-lg)] shadow-holo-lg" ref="agentSelectorDropdownRef" role="menu">
                  <div class="p-1.5 space-y-1">
                    <button
                      ...
                      role="menuitem"
                    >
                      <component :is="agentDef.icon || SparklesIcon" class="icon-base mr-2.5 shrink-0" :class="agentDef.iconClass || 'text-accent-light'" aria-hidden="true" />
                      <div class="text-left">
                        <span class="text-sm font-medium">{{ agentDef.label }}</span>
                        <p class="text-xs text-neutral-text-muted group-hover:text-neutral-text-secondary transition-colors">{{ agentDef.description }}</p>
                      </div>
                    </button>
                  </div>
                </div>
            ...
            <router-link to="/settings" class="header-icon-button" title="Application Settings">
              <Cog6ToothIconSolid class="icon-base" aria-hidden="true" />
              <span class="sr-only">Settings</span>
            </router-link>
            <button @click="handleLogoutHandler" class="header-icon-button text-error-light hover:text-error-DEFAULT" title="Logout">
              <ArrowRightOnRectangleIcon class="icon-base" aria-hidden="true" />
