 ...
 * @version 2.1.2 - TypeScript and import.meta.glob fixes.
 */
<template>
...
} from 'vue'; // Added onBeforeUnmount
...
const costStore = useCostStore(); // Initialized
const agentStore = useAgentStore(); // Initialized
const chatStoreInstance = useChatStore(); // Initialized and aliased
...
const getLanguageDisplayName = (langCode?: string): string => { /* ... same ... */ return langCode || ""};
...
const getAudioModeDescription = (mode: VoiceApplicationSettings['audioInputMode']): string => { /* ... same ... */ return ""};
const triggerRefreshAudioDevices = async (): Promise<void> => { /* ... same ... */};
const testMicrophone = async (): Promise<void> => { /* ... same ... */};
const stopMicrophoneTest = (): void => { /* ... same ... */};
const fetchCurrentSessionCost = async (): Promise<void> => { /* ... same ... */};
const handleResetSessionCost = async (): Promise<void> => { /* ... same ... */};

const handleLogout = (): void => {
 auth.logout(true).then(() => {
  costStore.$reset(); // Corrected: use instance
  chatStoreInstance.$reset();
  agentStore.$reset(); // Corrected: use instance
  uiStore.$reset();
...
const handleClearConversationHistory = async (): Promise<void> => {
 if (confirm("Are you sure you want to delete ALL your locally stored chat history for ALL agents? This action cannot be undone.")) {
  try {
   chatStoreInstance.clearAllAgentData(); // Corrected: use existing method
...
const exportAllSettings = (): void => { /* ... same ... */};
const triggerImportFile = (): void => { importSettingsInputRef.value?.click(); };
const handleImportSettingsFile = (event: Event): void => { /* ... same ... */};

onMounted(async () => {
 if (!auth.isAuthenticated.value) {
  const token = localStorage.getItem(AUTH_TOKEN_KEY) || sessionStorage.getItem(AUTH_TOKEN_KEY);
  if (token) { auth.checkAuthStatus(); }
...
watch(() => uiStore.currentThemeId, () => { /* ... */ }, { immediate: true });
