...
/**
 * @file Settings.vue
 * @description User-configurable settings page for the "Ephemeral Harmony" theme.
 * Allows users to customize appearance, general preferences, voice/audio settings,
 * memory management, session costs, security, and data management.
 * Changes are typically saved automatically via reactive stores and services.
 *
 * @component Settings
 * @props None
 * @emits None
 *
 * @version 2.2.2 - Corrected all TypeScript errors identified in the latest user feedback.
 * Ensured all variables are correctly typed and utilized.
 * Added comprehensive JSDoc comments.
 * Refined template for ARIA attributes and consistency.
 */
<template>
 <div class="settings-page-ephemeral">
  ...
  <main class="settings-main-content-ephemeral">
   ...
   <div class="settings-layout-grid-ephemeral">
    <SettingsSection title="Appearance" :icon="PaintBrushIcon" class="settings-grid-span-2" id="appearance-settings">
     <SettingsItem
    ...
    <SettingsSection title="General Preferences" :icon="WrenchScrewdriverIcon" class="settings-grid-span-2" id="general-preferences">
     <div class="settings-items-grid-ephemeral">
      <SettingsItem label="Default Assistant Mode" description="Initial assistant mode when the app starts." label-for="defaultModeSelect">
       <select id="defaultModeSelect" v-model="vcaSettings.defaultMode" class="select-input-ephemeral" aria-label="Default Assistant Mode">
        <option v-for="agentOption in availableAgentModeOptions" :key="agentOption.value" :value="agentOption.value">
         {{ agentOption.label }}
        </option>
       </select>
      </SettingsItem>
      <SettingsItem label="Preferred Coding Language" description="For code examples generated by assistants." label-for="defaultLanguageSelect">
       <select id="defaultLanguageSelect" v-model="vcaSettings.preferredCodingLanguage" class="select-input-ephemeral" aria-label="Preferred Coding Language">
     ...
     <SettingsItem
      ...
      label-for="generateDiagramsToggle" class="settings-item-spaced-ephemeral">
      <label class="toggle-switch-ephemeral" :aria-label="vcaSettings.generateDiagrams ? 'Diagram generation is ON' : 'Diagram generation is OFF'">
     ...
     <SettingsItem
      ...
      label-for="autoClearToggle" class="settings-item-spaced-ephemeral">
      <label class="toggle-switch-ephemeral" :aria-label="vcaSettings.autoClearChat ? 'Auto-clear chat is ON' : 'Auto-clear chat is OFF'">
    ...
    <SettingsSection title="Memory & Context" :icon="CpuChipIcon" class="settings-grid-span-2" id="memory-settings">
      ...
      >
        <label class="toggle-switch-ephemeral" :aria-label="useAdvancedHistoryManager ? 'Advanced memory management is ON' : 'Advanced memory management is OFF'">
      ...
      <div v-if="!useAdvancedHistoryManager" class="setting-subsection-ephemeral">
        ...
        <SettingsItem
          ...
          full-width-description
        >
          <input type="range" id="chatHistoryLength" v-model.number="chatHistoryCount" :min="MIN_CHAT_HISTORY_FOR_SLIDER" :max="MAX_CHAT_HISTORY_MESSAGES_CONFIGURABLE" step="2" class="range-slider-ephemeral" @input="updateRangeProgress($event.target as HTMLInputElement)" aria-valuemin="MIN_CHAT_HISTORY_FOR_SLIDER" :aria-valuemax="MAX_CHAT_HISTORY_MESSAGES_CONFIGURABLE" :aria-valuenow="chatHistoryCount">
        </SettingsItem>
      </div>
      <div v-if="useAdvancedHistoryManager" class="setting-subsection-ephemeral">
        <h4 class="subsection-title-ephemeral">Advanced Context Strategy</h4>
        <SettingsItem label="Context Strategy Preset" description="Choose a preset that dictates how chat history is selected and summarized for the LLM." label-for="advHistoryPreset">
         <select id="advHistoryPreset" v-model="advancedHistoryConfigLocal.strategyPreset" @change="onAdvancedPresetChange(($event.target as HTMLSelectElement).value as HistoryStrategyPreset)" class="select-input-ephemeral" aria-label="Advanced Context Strategy Preset">
        ...
        <div class="setting-subsection-ephemeral nested-subsection">
         <h5 class="subsection-title-ephemeral !text-sm !font-normal !opacity-80">Fine-tune Strategy Parameters</h5>
         <p class="subsection-description-ephemeral">Adjust parameters for the selected context strategy. These are advanced settings.</p>
         <SettingsItem label="Max Context Tokens for LLM" description="Target token count for the history context sent to the LLM (e.g., 4000, 8000, 16000)." label-for="advMaxContextTokens" class="settings-item-compact-ephemeral">
          <input type="number" id="advMaxContextTokens" v-model.number="advancedHistoryConfigLocal.maxContextTokens" min="500" max="128000" step="100" class="input-field-sm-ephemeral" aria-label="Maximum Context Tokens">
         </SettingsItem>
        <SettingsItem
          ...
          <input type="range" id="advRelevancyThreshold" v-model.number="advancedHistoryConfigLocal.relevancyThreshold" min="0.05" max="0.95" step="0.01" class="range-slider-ephemeral" @input="updateRangeProgress($event.target as HTMLInputElement)" aria-label="Relevancy Threshold" aria-valuemin="0.05" aria-valuemax="0.95" :aria-valuenow="advancedHistoryConfigLocal.relevancyThreshold">
        </SettingsItem>
        <SettingsItem
          ...
          <input type="number" id="advNumRecent" v-model.number="advancedHistoryConfigLocal.numRecentMessagesToPrioritize" min="0" max="50" step="1" class="input-field-sm-ephemeral" aria-label="Recent Messages to Prioritize">
        </SettingsItem>
         <SettingsItem
          v-if="isRelevancyStrategyActive"
          ...
          <input type="number" id="advNumRelevantOlder" v-model.number="advancedHistoryConfigLocal.numRelevantOlderMessagesToInclude" min="0" max="50" step="1" class="input-field-sm-ephemeral" aria-label="Maximum Relevant Older Messages">
         </SettingsItem>
         <SettingsItem
          v-if="advancedHistoryConfigLocal.strategyPreset === HistoryStrategyPreset.SIMPLE_RECENCY"
          ...
          <input type="number" id="advSimpleRecencyCount" v-model.number="advancedHistoryConfigLocal.simpleRecencyMessageCount" min="1" :max="MAX_CHAT_HISTORY_MESSAGES_CONFIGURABLE" step="1" class="input-field-sm-ephemeral" aria-label="Simple Recency Message Count">
         </SettingsItem>
        <div class="settings-items-grid-ephemeral settings-item-spaced-ephemeral">
          <SettingsItem label="Filter System Messages from History" description="Exclude past system messages (e.g., role: system) from being sent as context to the LLM." label-for="advFilterSystem" >
           <label class="toggle-switch-ephemeral" :aria-label="advancedHistoryConfigLocal.filterHistoricalSystemMessages ? 'Filtering system messages from history ON' : 'Filtering system messages from history OFF'">
             <input type="checkbox" id="advFilterSystem" v-model="advancedHistoryConfigLocal.filterHistoricalSystemMessages" />
             <span class="track"><span class="knob"></span></span>
           </label>
          </SettingsItem>
          <SettingsItem label="Characters per Token Estimate" description="Average characters per token, used for internal context size estimation (default: 4)." label-for="advCharsPerToken">
           <input type="number" id="advCharsPerToken" v-model.number="advancedHistoryConfigLocal.charsPerTokenEstimate" min="1" max="10" step="0.1" class="input-field-sm-ephemeral" aria-label="Characters per Token Estimate">
          </SettingsItem>
        </div>
        </div>
       <div class="setting-subsection-ephemeral nested-subsection">
         <h5 class="subsection-title-ephemeral !text-sm !font-normal !opacity-80">Strategy Management</h5>
         <div class="settings-actions-group-ephemeral">
          <button @click="resetCurrentAdvancedStrategyToDefaults" class="btn btn-secondary-outline-ephemeral btn-sm">Reset Current Strategy to Defaults</button>
          <button @click="resetAllAdvancedSettingsToGlobalDefaults" class="btn btn-secondary-outline-ephemeral btn-sm">Reset All Advanced to Global Defaults</button>
         </div>
       </div>
      </div>
    </SettingsSection>

    <SettingsSection title="Audio & Voice" :icon="SpeakerWaveIcon" class="settings-grid-span-2" id="audio-voice-settings">
     <div class="settings-items-grid-ephemeral">
       <SettingsItem label="Audio Input Device" :description="currentAudioDeviceName ? `Current: ${currentAudioDeviceName}` : 'Using system default input.'" label-for="audioDeviceSelect">
        <select id="audioDeviceSelect" v-model="vcaSettings.selectedAudioInputDeviceId" class="select-input-ephemeral" aria-label="Audio Input Device">
       ...
       <SettingsItem label="Audio Input Mode" :description="getAudioModeDescription(vcaSettings.audioInputMode)" label-for="audioModeSelect">
        <select id="audioModeSelect" v-model="vcaSettings.audioInputMode" class="select-input-ephemeral" aria-label="Audio Input Mode">
         <option value="push-to-talk">Push to Talk</option>
         <option value="continuous">Continuous Listening</option>
         <option value="voice-activation">Voice Activate ("V")</option>
        </select>
       </SettingsItem>
       <SettingsItem label="Speech Recognition (STT)" description="Whisper API for higher accuracy (may incur costs), Browser Web Speech for speed and no cost." label-for="sttPreferenceSelect">
     ...
     <div v-if="vcaSettings.autoPlayTts" class="setting-subsection-ephemeral">
       ...
       <div class="settings-items-grid-ephemeral">
        <SettingsItem label="TTS Provider" description="Service used for generating synthesized speech." label-for="ttsProviderSelect">
          <select id="ttsProviderSelect" v-model="vcaSettings.ttsProvider" class="select-input-ephemeral" aria-label="Text-to-Speech Provider">
           <option value="browser_tts">Browser Built-in (Free, voice quality varies by browser/OS)</option>
       ...
       <div class="settings-items-grid-ephemeral settings-item-spaced-ephemeral" v-if="isTTSSupportedBySelectedProvider">
        <SettingsItem label="TTS Speech Rate" :description="`Playback speed: ${vcaSettings.ttsRate.toFixed(1)}x (0.5x to 2.0x)`" label-for="ttsRateRange" full-width-description>
         <input type="range" id="ttsRateRange" v-model.number="vcaSettings.ttsRate" min="0.5" max="2" step="0.1" class="range-slider-ephemeral" @input="updateRangeProgress($event.target as HTMLInputElement)" aria-label="TTS Speech Rate" aria-valuemin="0.5" aria-valuemax="2" :aria-valuenow="vcaSettings.ttsRate">
        </SettingsItem>
        <SettingsItem v-if="vcaSettings.ttsProvider === 'browser_tts'" label="TTS Speech Pitch" :description="`Voice pitch: ${vcaSettings.ttsPitch.toFixed(1)} (0.0 to 2.0)`" label-for="ttsPitchRange" full-width-description>
         <input type="range" id="ttsPitchRange" v-model.number="vcaSettings.ttsPitch" min="0" max="2" step="0.1" class="range-slider-ephemeral" @input="updateRangeProgress($event.target as HTMLInputElement)" aria-label="TTS Speech Pitch" aria-valuemin="0" aria-valuemax="2" :aria-valuenow="vcaSettings.ttsPitch">
        </SettingsItem>
       </div>
     </div>
     <div v-if="vcaSettings.audioInputMode === 'voice-activation'" class="setting-subsection-ephemeral">
       ...
       <div class="settings-items-grid-ephemeral">
        <SettingsItem label="VAD Detection Sensitivity" :description="`Threshold: ${Math.round(vcaSettings.vadThreshold * 100)}% (Lower value is more sensitive to sound)`" label-for="vadThresholdRange" full-width-description>
         <input type="range" id="vadThresholdRange" v-model.number="vcaSettings.vadThreshold" min="0.01" max="0.5" step="0.01" class="range-slider-ephemeral" @input="updateRangeProgress($event.target as HTMLInputElement)" aria-label="VAD Detection Sensitivity" aria-valuemin="0.01" aria-valuemax="0.5" :aria-valuenow="vcaSettings.vadThreshold">
        </SettingsItem>
        <SettingsItem label="VAD Silence Timeout" :description="`Stop recording after ${vcaSettings.vadSilenceTimeoutMs / 1000} seconds of silence`" label-for="vadSilenceTimeoutRange" full-width-description>
         <input type="range" id="vadSilenceTimeoutRange" v-model.number="vcaSettings.vadSilenceTimeoutMs" min="500" max="5000" step="100" class="range-slider-ephemeral" @input="updateRangeProgress($event.target as HTMLInputElement)" aria-label="VAD Silence Timeout" aria-valuemin="500" aria-valuemax="5000" :aria-valuenow="vcaSettings.vadSilenceTimeoutMs">
        </SettingsItem>
       </div>
     </div>
     <div v-if="vcaSettings.audioInputMode === 'continuous' && vcaSettings.sttPreference === 'browser_webspeech_api'" class="setting-subsection-ephemeral">
       ...
       <div class="settings-items-grid-ephemeral">
        <SettingsItem label="Auto-send Transcription on Pause" description="Automatically send the transcribed text after a detected pause in speech." label-for="continuousAutoSendToggle">
         <label class="toggle-switch-ephemeral" :aria-label="vcaSettings.continuousModeAutoSend ? 'Continuous auto-send is ON' : 'Continuous auto-send is OFF'">
           <input type="checkbox" id="continuousAutoSendToggle" v-model="vcaSettings.continuousModeAutoSend" />
           <span class="track"><span class="knob"></span></span>
         </label>
        </SettingsItem>
        <SettingsItem v-if="vcaSettings.continuousModeAutoSend" label="Pause Detection Timeout" :description="`Wait for ${vcaSettings.continuousModePauseTimeoutMs / 1000}s of silence before auto-sending`" label-for="continuousPauseTimeoutRange" full-width-description>
         <input type="range" id="continuousPauseTimeoutRange" v-model.number="vcaSettings.continuousModePauseTimeoutMs" min="1000" max="10000" step="250" class="range-slider-ephemeral" @input="updateRangeProgress($event.target as HTMLInputElement)" aria-label="Continuous Mode Pause Timeout" aria-valuemin="1000" aria-valuemax="10000" :aria-valuenow="vcaSettings.continuousModePauseTimeoutMs">
        </SettingsItem>
       </div>
     </div>
     <div class="setting-subsection-ephemeral">
       ...
       <div class="mic-test-controls-ephemeral">
        <button @click="testMicrophone" :disabled="isTestingMic" class="btn btn-secondary-ephemeral btn-sm">
         <span v-if="isTestingMic" class="flex items-center"><SpinnerIcon class="mr-2" /> Testing Microphone... (5s)</span>
         <span v-else>Test Microphone Input</span>
        </button>
        <div v-if="micTestResult" class="mic-test-result-ephemeral" :class="micTestResultClass" role="status">{{ micTestResultMessage }}</div>
       </div>
       <div v-if="isTestingMic && micAudioLevels.length > 0" class="mic-audio-level-viz-ephemeral" aria-label="Live microphone audio levels">
        <div v-for="(level, index) in micAudioLevels.slice(-60)" :key="index"
             class="level-bar-ephemeral"
             :style="{ height: `${Math.max(1, level * 100)}%` }"
             :aria-valuenow="Math.round(level*100)" aria-valuemin="0" aria-valuemax="100" role="progressbar">
        </div>
       </div>
       <p v-if="isTestingMic" class="setting-description text-center text-xs mt-1">Speak into your microphone to see audio levels. Test runs for 5 seconds.</p>
     </div>
    </SettingsSection>

    <SettingsSection title="Session & Costs" :icon="CreditCardIcon" class="settings-grid-span-2" id="session-costs-settings">
     <div class="settings-items-grid-ephemeral">
       <div class="info-card-ephemeral">
         ...
         <button @click="handleResetSessionCost" class="btn btn-secondary-outline-ephemeral btn-sm mt-2.5 w-full">Reset Session Cost</button>
       </div>
       <div class="info-card-ephemeral">
     ...
     <SettingsItem label="Set Session Cost Limit ($)" :description="`Notification threshold for API usage: $${vcaSettings.costLimit.toFixed(2)}. Set to 0 for no limit (not recommended for public APIs).`" label-for="costLimitRange" class="settings-item-spaced-ephemeral" full-width-description>
       <input type="range" id="costLimitRange" v-model.number="vcaSettings.costLimit" min="0" max="50.00" step="0.50" class="range-slider-ephemeral" @input="updateRangeProgress($event.target as HTMLInputElement)" aria-label="Session Cost Limit" aria-valuemin="0" aria-valuemax="50" :aria-valuenow="vcaSettings.costLimit">
     </SettingsItem>
    </SettingsSection>

    <SettingsSection title="Security & Privacy" :icon="ShieldCheckIcon" class="settings-grid-span-2" id="security-privacy-settings">
     <SettingsItem label="Remember Login Across Sessions" description="Keep me logged in when I close and reopen the browser. Uses secure local storage." label-for="rememberLoginToggle">
       <label class="toggle-switch-ephemeral" :aria-label="rememberLoginLocal ? 'Remember login is ON' : 'Remember login is OFF'">
     ...
     <div class="danger-zone-ephemeral settings-item-spaced-ephemeral">
      <h4 class="subsection-title-ephemeral !text-[hsl(var(--color-error-h),var(--color-error-s),var(--color-error-l))]">Danger Zone</h4>
      <div class="settings-actions-group-ephemeral">
       <button @click="handleLogout" class="btn btn-danger-ephemeral btn-sm">
         <ArrowLeftOnRectangleIcon class="icon-xs mr-1.5" aria-hidden="true"/> Logout Current Session
       </button>
       <button @click="handleClearConversationHistory" class="btn btn-danger-outline-ephemeral btn-sm">
         Clear All Local Chat History
       </button>
      </div>
       <p class="setting-description text-xs mt-2">Logging out clears your current session data from this browser. Clearing chat history is permanent and cannot be undone for this device.</p>
     </div>
    </SettingsSection>

    <SettingsSection title="Data Management" :icon="ArrowDownTrayIcon" class="settings-grid-span-2" id="data-management-settings">
     <div class="settings-actions-group-ephemeral">
       ...
       <div class="relative w-full">
        <input ref="importSettingsInputRef" type="file" accept=".json" @change="handleImportSettingsFile" class="file-input-hidden-ephemeral" aria-label="Import settings file button">
        <button @click="triggerImportFile" class="btn btn-secondary-ephemeral w-full" aria-describedby="importSettingsInputRef">
         <ArrowDownTrayIcon class="icon-sm mr-2" aria-hidden="true" /> Import Settings from File
        </button>
       </div>
     </div>
...
const SpinnerIcon: VueComponent = {
  name: 'SpinnerIcon',
  render() {
    return h('svg', { class: "animate-spin h-4 w-4 text-current", xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24" }, [
...
watch(() => vcaSettings.useAdvancedMemory, (serviceVal) => {
  if (useAdvancedHistoryManager.value !== serviceVal) {
    useAdvancedHistoryManager.value = serviceVal;
  }
});

const advancedHistoryConfigLocal: Ref<AdvancedHistoryConfig> = ref({ ...advancedConversationManager.getHistoryConfig() });
...
const availablePresetDisplayNames = computed<Array<{key: HistoryStrategyPreset, name: string}>>(() => {
  return (Object.values(HistoryStrategyPreset) as HistoryStrategyPreset[]).map(value => {
    let name = value.replace(/_/g, ' ').replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase()).trim();
 ...
 return agentService
  ...
  .filter((agent) => {
    ...
    const coreAgentIds: AgentId[] = [
        'general_chat' as AgentId, 'diary' as AgentId, 'business_meeting' as AgentId,
        'system_designer' as AgentId, // Corrected typo
        'coding_interviewer' as AgentId, 'coding_assistant' as AgentId, 'tutor_agent' as AgentId
    ];
...
watch(advancedHistoryConfigLocal, (localConfigUpdate) => {
...
const onAdvancedPresetChange = (newPresetValue: HistoryStrategyPreset) => {
  ...
  const presetName = availablePresetDisplayNames.value.find(p => p.key === newPresetValue)?.name || newPresetValue;
  toast?.add({type: 'info', title: 'Context Strategy Updated', message: `Switched to "${presetName}". Settings adjusted.`});
};
const resetCurrentAdvancedStrategyToDefaults = () => {
  ...
  toast?.add({type: 'success', title: 'Strategy Reset', message: `Settings for "${currentPreset}" strategy reset to defaults.`});
};
const resetAllAdvancedSettingsToGlobalDefaults = () => {
  ...
  toast?.add({type: 'success', title: 'Advanced Settings Reset', message: 'All advanced memory settings reset to global defaults.'});
};
...
const micTestResult = ref<'' | 'success' | 'error_permission' | 'error_notfound' | 'error_overconstrained' | 'error_generic' | 'warning'>('');
...
const micTestResultMessage = computed<string>(() => {
  ...
  const messages = {
    ...
    'error_permission': 'Permission denied. Please enable microphone access in browser/system settings.',
    ...
    'warning': "Mic test complete, but very little sound was detected. Check mic levels and ensure it's not muted.",
    '': ''
  };
...
const micTestResultClass = computed<string>(() => {
  const classes = {
    ...
    'error_overconstrained': 'text-[hsl(var(--color-warning-h),var(--color-warning-s),var(--color-warning-l))]',
  ...
  return classes[micTestResult.value] || 'text-[hsl(var(--color-text-muted-h),var(--color-text-muted-s),var(--color-text-muted-l))]';
});
const isRelevancyStrategyActive = computed<boolean>(() => {
  const preset = advancedHistoryConfigLocal.value.strategyPreset;
  return preset === HistoryStrategyPreset.BALANCED_HYBRID || preset === HistoryStrategyPreset.RELEVANCE_FOCUSED || preset === HistoryStrategyPreset.MAX_CONTEXT_HYBRID;
});

const confirmAndGoBack = (): void => {
  toast?.add({ type: 'info', title: 'Settings Saved', message: 'Your preferences are saved locally.', duration: 3000 });
  router.push({ name: auth.isAuthenticated.value ? 'AuthenticatedHome' : 'PublicHome' });
};

const getLanguageDisplayName = (langCode?: string): string => {
  if (!langCode) return "Default";
  try { return new Intl.DisplayNames([langCode.split('-')[0]], { type: 'language' }).of(langCode) || langCode; }
  catch (e) { return langCode; }
};

const updateRangeProgress = (target: HTMLInputElement | null): void => {
 if (!target) return;
 const value = parseFloat(target.value); const min = parseFloat(target.min); const max = parseFloat(target.max);
...
const triggerRefreshAudioDevices = async (): Promise<void> => {
  toast?.add({type: 'info', title: 'Refreshing Devices', message: 'Updating microphone list... Permissions may be requested.'});
  await voiceSettingsManager.loadAudioInputDevices(true);
  if (voiceSettingsManager.audioInputDevices.value.length === 0) {
    toast?.add({type: 'warning', title: 'No Microphones', message: 'No microphones found. Ensure one is connected and enabled.'});
  }
};

const testMicrophone = async (): Promise<void> => {
  if (isTestingMic.value) return;
  isTestingMic.value = true; micTestResult.value = ''; micAudioLevels.value = []; micTestLocalMessage.value = '';
  try {
    if (micTestStreamLocal) { micTestStreamLocal.getTracks().forEach(track => track.stop()); micTestStreamLocal = null; }
    if (micTestAudioContext && micTestAudioContext.state !== 'closed') { await micTestAudioContext.close(); micTestAudioContext = null; }
    micTestStreamLocal = await navigator.mediaDevices.getUserMedia({ audio: vcaSettings.selectedAudioInputDeviceId ? { deviceId: { exact: vcaSettings.selectedAudioInputDeviceId } } : true });
    ...
    micTestAnalyser.fftSize = 256; micTestMicrophone.connect(micTestAnalyser);
    ...
    const draw = () => {
      ...
      micTestAnalyser.getByteFrequencyData(dataArray); let sum = 0;
      dataArray.forEach(v => sum += v); micAudioLevels.value.push(sum / dataArray.length / 255);
    };
    draw();
    setTimeout(() => {
      stopMicrophoneTest();
      if(micAudioLevels.value.reduce((s: number, v: number) => s+v, 0) < micAudioLevels.value.length * 0.01 && micTestResult.value === 'success') {
          micTestResult.value = 'warning';
          micTestLocalMessage.value = "Mic test complete, but very little sound was detected. Check mic levels and ensure it's not muted.";
          toast?.add({type:'warning', title: 'Low Audio Detected', message: micTestLocalMessage.value, duration: 6000});
      } else if (micTestResult.value === 'success') {
          micTestLocalMessage.value = "Microphone test finished. Levels displayed above.";
          toast?.add({type:'success', title: 'Mic Test Complete', message: micTestLocalMessage.value, duration: 4000});
      }
    }, 5000);
  } catch (err: any) {
...
const stopMicrophoneTest = (): void => {
  ...
  if (micTestStreamLocal) { micTestStreamLocal.getTracks().forEach((track: MediaStreamTrack) => track.stop()); micTestStreamLocal = null; }
  ...
  micTestMicrophone = null; micTestAnalyser = null; micTestAudioContext = null;
};

const fetchCurrentSessionCost = async (): Promise<void> => {
  try { if (auth.isAuthenticated.value) { await costStore.fetchSessionCost(); currentSessionCost.value = costStore.totalSessionCost; } }
  catch (error) { console.error("SettingsPage: Error fetching cost:", error); toast?.add({type: 'error', title: 'Cost Error', message: 'Could not retrieve session cost.'}); }
};
const handleResetSessionCost = async (): Promise<void> => {
  if (!confirm("Reset current session's API cost to $0.00? This doesn't affect overall billing.")) return;
  try { await costStore.resetSessionCost(); currentSessionCost.value = costStore.totalSessionCost; toast?.add({type: 'success', title: 'Session Cost Reset', message: 'Tracked session cost reset.'}); }
  catch (error) { console.error("SettingsPage: Error resetting cost:", error); toast?.add({type: 'error', title: 'Reset Failed', message: 'Could not reset cost.'}); }
};
...
const handleClearConversationHistory = async (): Promise<void> => {
 if (confirm("DANGER: Delete ALL local chat history for ALL assistants? This cannot be undone.")) {
  try { await chatStoreInstance.clearAllAgentData(); toast?.add({ type: 'success', title: 'History Cleared', message: 'All local chats deleted.' }); }
  catch (error) { console.error("SettingsPage: Error clearing history:", error); toast?.add({ type: 'error', title: 'Clear Failed', message: 'Could not clear history.' }); }
 }
};

const exportAllSettings = (): void => {
  try {
    const settingsToExport = { voiceAppSettings: voiceSettingsManager.settings, advancedMemoryConfig: advancedConversationManager.getHistoryConfig(), };
  ...
  } catch (error) { console.error("SettingsPage: Error exporting:", error); toast?.add({type: 'error', title: 'Export Failed', message: 'Could not export settings.'}); }
};
const triggerImportFile = (): void => { importSettingsInputRef.value?.click(); };
const handleImportSettingsFile = (event: Event): void => {
  const input = event.target as HTMLInputElement;
  if (!input.files?.length) { toast?.add({type: 'warning', title: 'No File', message: 'Please select a settings file.'}); return; }
  const file = input.files[0]; const reader = new FileReader();
  reader.onload = (e) => {
    try {
      const result = e.target?.result; if (typeof result !== 'string') throw new Error("File not string.");
      const importedData = JSON.parse(result); let applied = false;
      if (importedData.voiceAppSettings && typeof importedData.voiceAppSettings === 'object') {
        ...
        voiceSettingsManager.updateSettings(validVoiceSettings); applied = true;
      }
      if (importedData.advancedMemoryConfig && typeof importedData.advancedMemoryConfig === 'object') {
        ...
        advancedConversationManager.updateConfig(validAdvancedConfig); applied = true;
      }
      if (applied) {
        toast?.add({type: 'success', title: 'Settings Imported', message: 'Settings successfully imported.'});
        nextTick(() => {
          document.querySelectorAll<HTMLInputElement>('input[type="range"].range-slider-ephemeral').forEach(el => updateRangeProgress(el));
          useAdvancedHistoryManager.value = voiceSettingsManager.settings.useAdvancedMemory;
          if (!useAdvancedHistoryManager.value) chatHistoryCount.value = conversationManager.getHistoryMessageCount();
        });
      } else throw new Error("Invalid settings structure.");
    } catch (error: any) { console.error("SettingsPage: Import error:", error); toast?.add({type: 'error', title: 'Import Failed', message: `Import error: ${error.message}`});}
    finally { input.value = ''; }
  };
  reader.onerror = () => { toast?.add({type: 'error', title: 'File Read Error', message: 'Could not read settings file.'}); input.value = ''; };
...
onMounted(async () => {
 ...
 if (auth.isAuthenticated.value && auth.currentToken.value && (!mainApi.defaults.headers.common['Authorization'] || mainApi.defaults.headers.common['Authorization'] !== `Bearer ${auth.currentToken.value}`)) {
...
onBeforeUnmount(() => { stopMicrophoneTest(); });
