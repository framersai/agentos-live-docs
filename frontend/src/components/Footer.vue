      ...
      <div class="flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="flex flex-col items-center md:items-start">
          <div class="flex items-center gap-2">
            <img src="../assets/logo.svg" alt="Logo" class="w-5 h-5" />
      ...
      <TransitionGroup name="expand">
        <div v-if="logsOpen" key="logs" class="mt-4 p-3 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden">
          <div class="flex items-center justify-between mb-2">
            ...
            <button @click="clearLogs" class="text-xs text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
              Clear
            </button>
          </div>
          ...
          <div class="logs-container h-32 overflow-y-auto bg-gray-50 dark:bg-gray-900 rounded border border-gray-200 dark:border-gray-700 p-2">
            <div v-for="(log, index) in logs" :key="index" class="text-xs font-mono mb-1">
              <span :class="getLogLevelClass(log.level)">{{ log.timestamp }} [{{ log.level }}]</span> {{ log.message }}
            </div>
...
const sampleLogs = [
  ...
  { level: 'info', message: 'Session cost updated', timestamp: '2025-05-21 11:35:10' },
];
...
onMounted(() => {
  ...
  // Add new log every 30 seconds for demo purposes
  setInterval(() => {
