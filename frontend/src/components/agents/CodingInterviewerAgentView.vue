// File: frontend/src/components/agents/CodingInterviewerAgentView.vue
/**
...
import { UserCircleIcon, ChatBubbleLeftRightIcon, PaperAirplaneIcon } from '@heroicons/vue/24/outline'; // UserCircleIcon was specified

type InterviewStage = 
    | 'initial'             // Agent greets, ready to start
    | 'problem_pending'     // Waiting for LLM to generate/present problem
    | 'problem_presented'   // Problem is shown, user can ask clarifying Qs or submit solution
    | 'awaiting_solution'   // User indicated they are ready to submit/is submitting
    | 'evaluating_solution' // LLM is evaluating submitted solution
    | 'feedback_displayed'  // Feedback is shown, user can ask about feedback or next problem
    | 'session_ended';
...
const currentProblemStatement = ref<string | null>(null); // For main content
const userSolutionInput = ref<string>(''); // For a textarea where user types their code
...
const fetchSystemPrompt = async () => {
    // ... (similar to GeneralAgentView, loads coding_interviewer.md)
     if (props.agentConfig.systemPromptKey) {
        try {
            ...
            currentAgentSystemPrompt.value = "You are a Coding Interviewer."; // Fallback
        }
    } else {
        currentAgentSystemPrompt.value = "You are a Coding Interviewer.";
    }
};
...
const startInterviewOrNextProblem = async () => {
    ...
    // Construct message to LLM to provide a problem
    const promptForProblem = "Please provide the next coding problem statement, including constraints and expected input/output formats. Present it clearly for the candidate.";
    chatStore.addMessage({
        id: `internal-getproblem-${Date.now()}`, role: 'user', content: promptForProblem,
        timestamp: Date.now(), agentId: props.agentId,
    });
    await callInterviewerLLM(promptForProblem, true); // isSystemAction = true means response primarily updates main content
};
...
const submitSolution = async () => {
    ...
    chatStore.addMessage({
        id: `user-solution-${Date.now()}`, role: 'user', content: solutionText,
        timestamp: Date.now(), agentId: props.agentId,
    });
    ...
    userSolutionInput.value = ''; // Clear input
    await callInterviewerLLM(solutionText, true); // Response will be feedback
};

const handleNewUserInput = async (text: string) => { // For clarifying questions or general chat during interview
    if (!text.trim() || isLoadingResponse.value) return;
    chatStore.addMessage({
        id: `user-clarify-${Date.now()}`, role: 'user' as 'user', content: text,
        timestamp: Date.now(), agentId: props.agentId,
    });
    await callInterviewerLLM(text, false); // isSystemAction = false implies more conversational reply
};

async function callInterviewerLLM(userInput: string, isSystemDrivenUpdate: boolean) {
    isLoadingResponse.value = true;
    // If not system driven, show thinking in chat, else main content is already showing loading
    if (!isSystemDrivenUpdate) {
        // Simple chat reply expected
    }
    ...
    try {
        ...
        let finalSystemPrompt = currentAgentSystemPrompt.value
            ...
            .replace(/{{USER_QUERY}}/g, userInput) // Current interaction point
        ...
        const historyForApi = await chatStore.getHistoryForApi(
            props.agentId, userInput, finalSystemPrompt,
            props.agentConfig.capabilities?.maxChatHistory || 20 // Interviewer might need more context
        );
        ...
        const payload: ChatMessagePayload = {
            messages: messagesForLlm, mode: props.agentConfig.systemPromptKey, language: preferredLang,
            generateDiagram: props.agentConfig.canGenerateDiagrams && voiceSettingsManager.settings.generateDiagrams,
            userId: 'private_user_interviewer', conversationId: chatStore.getCurrentConversationId(props.agentId),
        };
        
        const response = await chatAPI.sendMessage(payload);
        const assistantMessageContent = response.data.content || "An issue occurred with my response.";

        chatStore.addMessage({
            id: `asst-interview-${Date.now()}`, role: 'assistant', content: assistantMessageContent,
            timestamp: Date.now(), agentId: props.agentId, model: response.data.model, usage: response.data.usage,
        });

        if (interviewStage.value === 'problem_pending') {
            currentProblemStatement.value = assistantMessageContent; // Assume LLM gives just the problem statement
            chatStore.updateMainContent({
        ...
        } else if (interviewStage.value === 'evaluating_solution') {
            chatStore.updateMainContent({
                agentId: props.agentId, type: 'compact-message-renderer-data', data: assistantMessageContent,
                title: `Feedback on Your Solution`, timestamp: Date.now()
            });
            interviewStage.value = 'feedback_displayed';
        } else { // General clarification or chat during interview
             // Let this response just appear in chat log
        }
  ...
  <div class="coding-interviewer-view flex flex-col h-full w-full overflow-hidden">
    <div class="agent-header-controls p-2 px-3 border-b border-purple-500/20 dark:border-slate-700/50 flex items-center justify-between gap-2">
        <div class="flex items-center gap-2">
            <UserCircleIcon class="w-5 h-5" :class="props.agentConfig.iconClass" />
            <span class="font-semibold text-sm">{{ props.agentConfig.label }}</span>
            <span class="text-xs px-2 py-0.5 bg-slate-700 rounded capitalize">{{ interviewStage.replace('_', ' ') }}</span>
        </div>
        <div>
            <button 
    ...
    <div class="flex-grow relative min-h-0">
        ...
        <template v-if="mainContentToDisplay && !isLoadingResponse">
            ...
            />
            <div v-else class="prose dark:prose-invert max-w-none p-4 md:p-6 flex-grow overflow-y-auto h-full"
...
.loading-overlay-interviewer {
    ...
    z-index: 10; color: theme('colors.red.200');
}
.spinner-interviewer {
    width: 36px; height: 36px; border: 4px solid rgba(248, 113, 113, 0.2); /* red */
...
.btn.btn-xs { /* For smaller buttons if needed */
    @apply py-1 px-2 text-xs;
}
/* Styles for dropdown if any, copied from Tutor for consistency */
