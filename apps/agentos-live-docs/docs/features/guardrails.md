...
await agent.initialize({
  ...config,
  manifest: { packs: [{ factory: () => piiPack }] },
  // Additional guardrails (ContentPolicyGuardrail, CostCeilingGuardrail, etc.)
  // can be registered as further extension packs alongside the PII pack.
...
{
  "defaultPolicy": "deny",
  "rules": [{ "pattern": "~/workspace/**", "read": true, "write": true }]
}
```
...
- [Human-in-the-Loop](/docs/features/human-in-the-loop)
- [Agent Communication](/docs/features/agent-communication)
