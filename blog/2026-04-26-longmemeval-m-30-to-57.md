...
The 45.4% headline below has been superseded TWICE in the same day. First by **57.6% [53.2%, 61.8%]** with sem-embed + per-category reader router on top of the M-tuned config ([prior post](2026-04-29-longmemeval-m-57-with-sem-embed.md), Phase B N=500). Then by the current **[70.2% [66.0%, 74.0%] M headline](2026-04-29-longmemeval-m-70-with-topk5.md)** which lowered reader-top-K from 50 to 5 — **+24.8 pp over the 45.4% baseline below**, **+12.6 pp over the 57.6% intermediate**, both with CIs non-overlapping. **+4.5 pp above the LongMemEval paper's academic-baseline ceiling (65.7%)**. Cost per correct dropped 17× ($0.1348 → $0.0078). **agentos-bench is the first open-source memory library on the public record with end-to-end LongMemEval-M QA accuracy above 65% with full methodology disclosure**. The retrieval-precision diagnosis below — wider rerank pool + larger reader-top-K + HyDE — is now the M-tuned config baseline that two further axes (sem-embed + top-K=5) layered on top of.
:::
