...
The agentos-bench published number on LongMemEval-S Tier 3 min-cost was **76.6% [72.8%, 80.2%]** at full Phase B N=500. Mid-pack against published vendor numbers: Mastra Observational Memory at 84.2% (gpt-4o), Supermemory at 81.6% (gpt-4o), [EmergenceMem at 86%](https://www.emergence.ai/blog/sota-on-longmemeval-with-rag).

That published 76.6% was running against the bench's `CharHashEmbedder`, a lexical-hash stub the bench falls back to when no embedder is configured. **It is not what real consumers wiring `@framers/agentos` memory primitives use in deployment.**

Measuring Tier 3 min-cost with the same configuration but `text-embedding-3-small` as the embedder (the documented production path) lands the result at **83.2% [79.8%, 86.4%]** at full Phase B N=500. Weighted aggregate (true category distribution): **84.2% [81.0%, 87.2%]**. Cost per correct: **$0.0521**, slightly cheaper than the CharHash baseline despite the embedding cost. The semantic retrieval finds answers lexical hashing missed, lifting more cases to "passed."
...
| Metric | CharHash baseline (prior) | text-embedding-3-small (this post) | Δ |
|---|---:|---:|---:|
| Aggregate accuracy | 76.6% [72.8%, 80.2%] | **83.2% [79.8%, 86.4%]** | +6.6 pp |
| Weighted (true distribution) | reference | **84.2% [81.0%, 87.2%]** | reference |
| Cost per correct | $0.0580 | **$0.0521** | -10% |
| Total LLM cost | reference | $21.66 | reference |
| Avg latency | 16,130 ms | 73,234 ms | +4.5x |
...
The lift concentrates exactly where semantic embeddings should matter: **temporal-reasoning** (+14.5 pp) and **multi-session** (+14.5 pp). These are the categories that depend on finding paraphrase-rich and multi-hop bridges across sessions, which lexical hashing fundamentally cannot do.
...
A substantial lift over the recall and NDCG the CharHash run produced.
...
The bench's `--embedder-model text-embedding-3-small` flag exposes the same path the documented production wiring uses. It was never the default for the published Phase B runs. The 76.6% measured "agentos at minimum config" rather than "agentos as deployed."

This is not unique to AgentOS. Every benchmark harness has a "what's running by default" question, and the answer determines what number gets published. The 76.6% number stays on record as the bench-default reference point so the gap is auditable.
...
| Mem0 v3 (managed platform) | 93.4% | not specified |
...
For an open-source library at the gpt-4o reader, **83.2% is genuinely competitive**. Beats Supermemory's gpt-4o number by +1.6 pp and lands within statistical noise of Mastra OM gpt-4o (84.2% point estimate; our CI overlaps with where theirs likely sits). Cost per correct is $0.052, competitive with vendor-published numbers despite shipping as an open library rather than a managed platform.
...
- **Same judge harness** (`gpt-4o-2024-08-06` with rubric `2026-04-18.1`); judge false-positive rate **1% [0%, 3%]** at n=100 ([Stage G probe](https://github.com/framersai/agentos-bench/blob/master/docs/SESSION_2026-04-24_TRANSPARENT_NEGATIVES.md)).
- **Bootstrap 95% confidence interval at 10,000 resamples.** Most vendors do not publish CIs.
...
- Mastra OM's 94.9% uses `gpt-5-mini` as the OM observer; their 84.2% number uses `gpt-4o`. Cross-provider observer setups are not single-provider reproducible.
...
The bench's `--embedder-model text-embedding-3-small` flag had existed for a while. What was missing was making the cache fingerprint include the embedder, so that runs with different embedders could not accidentally share cached answers. We caught this when the M Phase B with semantic embedder returned the IDENTICAL aggregate as the prior CharHash run with `totalUsd=$0.00`, a 100% cache hit on incompatible cached answers.

That is now fixed in [agentos-bench commit `215358817`](https://github.com/framersai/agentos): the `case-run` and `case-result` cache fingerprints partition by embedder model id. Prior CharHash-fallback runs keep their cache hashes (the fingerprint is conditional and omitted when no embedder is set), so existing CI runs are not disrupted.
...
The semantic-embedder lift on TR and MS specifically (+14.5 pp each) matches the [LongMemEval paper's](https://arxiv.org/abs/2410.10813) own per-category retrieval-error analysis (Wu et al., §5.2). The paper notes that lexical retrieval underperforms semantic retrieval most on questions requiring multi-session synthesis or temporal anchoring across paraphrases. CharHash is a lexical-hash stub. Replacing it with a transformer embedder targets exactly the failure modes the paper identified.

The two-axis decomposition (retrieval embedding + decision-making module) follows the **CoALA framework** for cognitive architectures (Sumers et al., [arXiv:2309.02427](https://arxiv.org/abs/2309.02427)), which separates memory access from decision logic. AgentOS's `MemoryRouter` and `ReaderRouter` are CoALA-style modules; this post measures the retrieval-side calibration.
...
2. **Semantic embedder on M Phase B.** The M-tuned 45.4% number was also CharHash. Semantic embedder + M-tuned flags compounded should lift M materially.

Both will land in v2 alongside the validated S 83.2% headline as the v1 publication anchor.

## Related

- [85.6% on LongMemEval-S at $0.009/correct](2026-04-28-reader-router-pareto-win.md). The follow-up Pareto-win headline.
