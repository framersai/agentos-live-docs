...
- **Workflows exist but do not run** - `WorkflowDefinition` lives in `packages/agentos/src/orchestration/workflows/WorkflowTypes.ts:77` and `AgentOS.startWorkflow` (packages/agentos/src/api/AgentOS.ts:917) persists instances, yet there is no executor loop (`WorkflowEngine.startWorkflow`, packages/agentos/src/orchestration/workflows/WorkflowEngine.ts:182`).
